...
未達はASIO実装／配布ライセンス選択、同一device hot-plug自動復帰、live spectrumのengine適用ack、capture-to-analysis／engine／pixel latency percentile、16 bandを任意の照明・映像parameterへ割り当てるAudio Reactive Rack、kick／snare／centroid／density等の高次descriptor、実機一時間比較である。したがってTouchDesigner同等以上はまだ主張しない。受入境界は`qa/AUDIO_REACTIVE_VJ_ACCEPTANCE.md`と`qa/ASIO_INPUT_ACCEPTANCE.md`を正とする。
