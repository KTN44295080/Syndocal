...
- 未着手で残る監査項目: F8（Matrix列密度: Daslight約12列 vs Syndocal 7列の実測差を確認済み、セル情報量トレードオフ込みでT25候補）→ 2026-08-09クローズ（T27-B 156px列で実質解消、T25系末尾の再計測記録参照）
...
- **T25-D ライブデスクヘッダの1行化 ✅完了 2026-07-29（7f73533、Codex実装＋control-mode再交渉ラウンド）** — ヘッダ32px単一行（9+2+1アクション配列）、ラベル/準備完了/照明readout撤去（readoutはステータスポップアップへ）、マトリクス/キューパッド切替＋ステータスを右端へ、killZone断言（KILL3種ハッチ+寸法）不変、解放高はマトリクスへ（494→527px可視 at 1920x1080）。監督フルマトリクスが旧ヘッダ構造参照のcontrol-mode-surface 35件を検出→再交渉（21フェーズ×5解像度green）。live-desk-header焦点契約新設。（本記録は2026-08-09 Fable引き継ぎ監査での事後追記——実装は7/29に着地済みで計画書マークのみ欠落していた）元仕様（2026-07-29ユーザー追加指摘）: 「ここも二行である意味あるか？」——1行目（ライブデスクラベル/マトリクス・キューパッド切替/照明・30シーン/ステータス/準備完了）と2行目（トランスポート＋KILLゾーン）を統合する。ラベルは削除（文脈自明）、「準備完了」は最下部ステータス行と完全重複のため削除、「照明・Nシーン」はステータスポップアップへ畳む。統合後: [戻る|GO|Pause Fade|Play Timeline|DMX BO|映像BO|全BO|全消去|フラグをクリア]＋右端[マトリクス/キューパッド|ステータス]の1行。killZone断言（KILL3種・ハッチング・寸法）は維持し、行構成変更分のみ再交渉（全列挙）。解放高はマトリクスへ。
...
- F8（Matrix列密度~1.7倍差）はT25-Aの行整理後に再計測して列幅コンパクト化を判断。→ **✅クローズ 2026-08-09（Fable再計測）**: T27-B（05fa507）の156px固定列で可視7→10.7列、収納カード50px高で8.7枚/列となり、同時可視セル概算はSyndocal~93 vs Daslight~72（詳細実測値は`qa/OPERATOR_AUDIT_2026-07-29.md` F8行）。追加の列幅圧縮は不実施と判断。
...
## T29: セットアップ画面のDaslightゾーニング簡素化（2026-07-31ユーザー指示）

ユーザー評価: 「セットアップ画面も明らかに煩雑。DasLightのそれですでに完成している」。
原則: **常時表示をDaslightの4ゾーン水準へ削減、機能は削らず文脈表示/折り畳みへ**。
Daslightの正: ①ライブラリ（ツリー+検索）②パッチグリッド+最小フォーム（ユニバース/開始アドレス/台数/PATCH）
③下段ステージ+グループ+SELECTIONS ④リミット（選択時に意味を持つ右パネル）。

- **T29-A パッチ画面ゾーニング**: Patch Source（GDTFローカル/Share）をライブラリペインの折り畳み
  「インポート」へ統合。中央はグリッド+最小パッチフォームのみ（ラベル/グループ編集は選択文脈へ）。
  ユニバース概要バーはグリッドヘッダへ圧縮。灯体セットアップ右ペインは**選択時のみ**表示
  （未選択時は空状態ガイダンス1行）。
- **T29-B 下段ステージ帯簡素化**: スナップ プリセット列→単一ドロップダウン、レイヤーチェック群→
  コンパクトトグル（既存ホットキー維持）、ステージオブジェクトエディタと選択フラグ→選択時のみ。
- **T29-C サブタブ統合検討**: ライブラリ/プロファイル/パッチ3枚のDaslight型1枚統合。
  ビューポートフェーズ契約の大規模再交渉を伴うため、A/B着地後に規模を再見積もりして着手判断。

各スライスはハーネス断言の再交渉列挙必須（削除禁止・同名断言の他フェーズ掃引込み）。

### T29追補（2026-07-31ユーザー指示・最優先原則）

1. **固定レイアウト契約「上1・下2」**: 選択した項目によって画面レイアウトが変化してはならない。
   ペイン枠（位置・寸法）は常時一定で、選択状態が変えてよいのは**各ペインの中身だけ**。
   空状態はペイン内の案内行で表現し、ペインの消滅・グリッド再流動は禁止。
   T29-A/Bの文脈表示はこの契約へ矯正する（右ペイン枠は常設、editor⇔guidanceの中身切替のみ）。
   ハーネスに「選択前後でペインrect不変」の断言を追加する。
2. **照明マッピングのセットアップ統合**: 灯体の配置・回転・整列はDaslight同様セットアップ画面の
   下段ステージで完結させる。照明用途に別タブは持たない。
3. **マッピングタブの映像専用化**: 投影面編集（ワープ/キーストン等・Syndocal固有）は画面を
   分けてよいが、レイアウト枠は同じ「上1・下2」を守る。

### T29完了記録（2026-07-31）

- T29-A（85f83a6）: パッチ画面ゾーニング——常時表示150→89（未選択、−40.7%）/128（選択中）。
- T29-B（25b66df）: 下段帯密度106→94/75。固定「上1・下2」枠契約を恒久ゲート化
  （ピック遷移rect±0.5px不変断言＋Mapping→I/O→Patchシーケンス検証新設）。
- T29-C（3b7d0aa）: マッピング→「ステージ」改名（実体=ステージ帯の全画面ビュー。内部識別子不変）。
- 実機受入はユーザー指示により保留中（ゲームセッション中）。ビューポート追従修正（c945bb2）分と
  合わせて次のネイティブサイクルで実施。
