...
- Daslight `CSweepEffect` was recovered from the same installed binary: constructor `0x140355D30`, vtable `0x140696440`, evaluator `0x1403665A0`, `Direction Change` property ID10/default false and one-row alternate 180-degree transitions. Syndocal now matches its integer hard-boundary/current-next palette algorithm for VALUE. The exact evidence and deliberate 2D/DVC boundaries are recorded in `qa/DVC_SWEEP_SOURCE_PARITY.md`.
- Current gates: DVC 56/56; locked workspace 1,188 passed / 13 existing hardware, GPU or long-duration ignores / zero failures; DVC MIDI 39 assertions; DVC DMX 33 assertions; backend operator contract 364 commands / 204 literal frontend calls / 162 transactional mutations; localization 3033/3033; VALUE source contract, focused protocol/engine tests, five-size Scene Settings DOM path and production frontend build all PASS. `pnpm --dir app tauri build --no-bundle` passed after the exact-checkout process guard reported zero running instances. The 2026-08-09 22:23:34+09:00 release executable is 39,084,544 bytes, version 1.1.0, SHA-256 `627CF6E74032D753455034A0F5043AF11ECD6CB247F1AB4B5E41D9A7B7382C48`. It was not launched because another task owns the desktop surface; current native-window verification is not claimed.
- Remaining saved-DVC/software evidence is explicit: a targeted VALUE FX saved specimen (including ID625 Sweep serialization), any COLOR MAPPINGS saved specimen and a Move rack with `BEAMID>0`. Arbitrary 2D MAPPINGS Sweep orientation remains separate from the proven one-row VALUE path. Physical fixtures, MIDI feedback hardware, DMX node/electrical output, commercial visualizer/live LAN and Mac/Linux remain external acceptance boundaries.
