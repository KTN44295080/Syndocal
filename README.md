...
- Show a lightweight Setup 2D Mapping stage with fixture/projector surfaces, selected-fixture highlighting, group/type filtering, and dimmer/color/Pan-aware beam wedges.
...
The 2D Mapping stage is intentionally frontend-only for now: it maps patched fixture `x/z` positions and video-output `stage_x/stage_z` surfaces into a top-down stage grid, uses current `Dimmer`/RGB attributes when present, highlights the selected fixture, and can place or rotate the selected fixture from the stage tools. The mapping editor also provides group/type/search filtering, projector surface selection, ratio/aspect/lens/keystone controls, and Line/Grid/Circle layout helpers that operate on the selected group filter or the full patch. The embedded SVG/isometric 3D UI is deferred; the Rust `visualizer` crate remains as a renderer-facing boundary for later native or external visualization work.
