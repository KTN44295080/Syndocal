...
現在の照明UI比較トランシェでは、Daslight 5で先に見ていた`Moving Chaser Rnd`をPan/Tilt／Chaser／Moveの証拠から除外した。これはDimmer effectだった。確認済みのローカル証拠は、最大化1920x1032で開いたSTATIC Position scene `Moving-Pos > Center-Up (2)`だけであり、scene/group matrixとstageを残したまま、compactな2D Pan/Tilt pad、正確なPan/Tilt値、独立axis faderを表示する。Syndocalのstatic Positionを、左の常設2D pad／Pan-Tilt axis faderと、右のPosition／Movement Limits tabからなるcompact consoleへ再構成した。Position面は直接値、Nudge、3x3 target、Mirror、Favoriteを同じ面へ置き、Limits面は可動域map、数値、invert／swap、適用操作を保持する。1920x1080では913x317のsurface、289x307のpad、499pxのtool deckに全主要操作とFavorite 3件が初期表示内で収まり、内外横overflow 0、追加縦scroll 0を確認した。1280x720／1366x768はpane内scrollで末尾操作まで到達し、2048x1152を含む全4 viewportでdocument/app overflow 0、Limits切替、keyboard tab移動を通過した。dynamic Moveは公式manualのeditable Pan/Tilt path／point、translate／resize／rotate、Absolute／Relative、live direction／phase／sizeを別の未達境界として扱う。

Cueは選択したEffectごとのON／OFFを`CueEffectTarget`として保存し、GO／Back／直接発火／pre-wait完了／follow進行時に一度だけ適用する。リスト外のEffectは変更しない。Store Scopeへ`Effects Only`を追加し、Lighting／Video／Selected Fixture／Selected Groupでは対象範囲内だけを更新して範囲外のRecallを保持する。Cue編集は`Save Details`（メタデータのみ）、`Save Recall`（Effect Recallのみ）、`Update Look`（現在のStore Scopeを再取得）へ分離し、Live面の`Edit Cues`から到達できる。作成／Details／Look更新／Recall保存とEffect ON／OFF切替はsnapshot公開ack後に成功し、Detailsはmetadata／Part／Mark／MIBを単一mutationで確定、非Main Cue List作成も単一engine commandで割当まで確定する。明示full snapshotは250ms delta pollより優先して直列・batch化し、full同期中のdeltaを破棄してdirty判定とdraft同期を守る。旧`.sdc`は空Recallへ互換移行し、Effect削除時は参照を清掃する一方、ユーザーが開く`.sdc`の未知／重複Effect参照は黙って修復せず検証errorとして拒否する。Cueごとの閉じたRecall editorは行DOMを生成せず、開いたeditorも検索付き48件pageへ制限し、500 Effect×12 Cueを全3 viewportで検査する。Recallは現時点では即時ON／OFFであり、Effect parameterのCue別morph／fade、source-linked Scene Block、画像／映像RGB sampling、校正済み多原色、専用Move／Curve／Value／Mapping engineは未達である。

旧Library／Form／Graph／Stack縦積みUIは、Effect Library／Inspector／Live Rackを同時表示する全画面3-pane deskへ刷新し、適用操作を各pane下端へ固定、Stack／Graphsをrack tab化した。Color inspectorはgradient、stop color／hex／position／並べ替え／追加削除、algorithm／interpolation／clock／spreadを提供し、scalar Attribute／waveform／video targetを表示しない。Chaser inspectorはpreview、4 direction、Pixels on、Wings、Size、Fading／Overlap、Spread／Phase、Feature range／coverage、ordered stepとgap編集を同じ作業面へ置く。1280x720／1366x768／1920x1080／2048x1152で外側scrollなし、3 pane／2 action dock／rack tab、8 family button、13 card、3 target-required card、Colorの3→4 stop→削除／Random／HSV longest／gradientに加え、Chaserの6 step／2 Feature／Reverse／active width／Size／Fading／6 step置換、各editor横overflow 0を検査し、2082/2082の日本語coverageを維持する。詳細な受入境界は`qa/LIGHTING_FX_ACCEPTANCE.md`と`qa/CHASER_FX_ACCEPTANCE.md`へ固定した。
...
- 最大化1920x1080を情報密度・一覧性・通常運用の主visual gateとし、1280x720／1366x768は小画面containment fallback、2048x1152は拡張上限回帰として別に維持する。fallback通過だけでUIデザイン合格としない。
