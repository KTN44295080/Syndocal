...
|---|---|---|---|
| Live presentation | [Perform Mode](https://docs.derivative.ca/Perform_Mode) renders one selected Window COMP without the network editor; Windows full-screen exclusive can avoid compositor stutter | A dedicated VJ Desk, automatic maximize, exact F11 1920x1080, separate Program outputs, six-layer quick banks and a synchronized all-output state rail; no exclusive-swapchain proof | Prepared-show operation is comparable in shape and now has a populated operator-state gate, but exclusive presentation and performance parity are unproven |
...
| Runtime profiling | [Performance Monitor](https://docs.derivative.ca/Performance_Monitor_Dialog) records cook/UI/display events and triggered slow frames; [Perform CHOP](https://docs.derivative.ca/Perform_CHOP) exposes FPS, cook state/time slice and memory; [Probe](https://docs.derivative.ca/Palette%3Aprobe) visualizes CPU/GPU time and memory | Engine, audio callback/queue, DMX, decoder and selected renderer telemetry exist. ISF now reports pipeline count, last executed stack-stage count, aggregate stack render microseconds and layer/stage-labelled errors, but there is no single production frame graph covering capture → analysis → engine → GPU → display with comparable history | TouchDesigner has the broader interactive profiling toolkit. Aggregate stack timing is useful but not per-stage GPU timing or end-to-end history; unified frame telemetry remains required before claiming easier diagnosis or lower runtime cost |
...
| GPU effect chain | TOP networks keep textures GPU-resident through arbitrary operator chains and support feedback/multipass structures | Each layer can run up to eight ordered single-pass ISF stages. One host upload feeds GPU-local ping-pong textures and one final readback; bad stages are skipped with stage-local diagnostics while valid stages continue. The returned CPU RGBA frame is then re-uploaded by the separate native-output device | Syndocal now has a real bounded multi-effect GPU chain, not a single FX slot. TouchDesigner still leads decisively in zero-copy graph residency, arbitrary topology, feedback/multipass and operator-library breadth |
...
| Authoring | General operator network and custom component model | Fixed Source → Transform → Output graph plus an ordered maximum-eight single-pass ISF stack per layer. The stack has targeted add/reorder/bypass/reset/remove/control commands and a lazy selected-stage editor, but no arbitrary texture graph, fan-out or reusable node tool authoring | TouchDesigner leads decisively in general authoring; the bounded stack is a prepared-show specialization |
...
7. Keep the implemented maximum-eight ordered ISF evaluation (one upload,
   GPU-local ping-pong, one readback) and remove its final headless readback ->
   separate-device native re-upload boundary with a measured shared-texture or
   unified-device render graph. Add per-stage GPU timing and end-to-end
   decode/effect/composite/output telemetry before making a performance claim.
...
- It has a bounded eight-stage GPU-local ISF chain, but does not yet provide
  TouchDesigner-class general visual programming, shared-texture/zero-copy
  output residency, distribution-ready and cross-hardware ASIO/multichannel
  breadth, matched dataset accuracy, GPU graph/library breadth or proven
  end-to-end performance parity.
