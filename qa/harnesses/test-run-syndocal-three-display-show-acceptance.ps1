...
$script:GoodVersion = "1.2.0-alpha.13"
...
function Invoke-FocusedChecks {
  ...
  try {
    ...
    $checks.Add([pscustomobject]@{ Name = "dirty checkout is rejected before display acceptance"; Run = {
      Set-TestSeam "Test-ThreeDisplayCheckoutClean" { param($CheckoutRootPath) throw "Fail closed: exact alpha.13 artifact acceptance requires a clean checkout; git status reported 1 change(s)." }
    ...
    $checks.Add([pscustomobject]@{ Name = "non-alpha.13 configuration is rejected"; Run = {
      Assert-Throws { New-ThreeDisplayConfiguration -IsApply $true -ExecutablePath $script:ExpectedPath -Sha256 $script:GoodHash -ProductVersion "1.2.0-alpha.12" -GitHead $script:GoodHead -EditorIdentity $script:EditorIdentity -LedIdentity $script:LedIdentity -ProjectorIdentity $script:ProjectorIdentity -LedId 41 -LedLabel "LED Program" -ProjectorId 42 -ProjectorLabel "Projector Program" -CdpPort 5189 -IntervalMs 200 -Attempts 3 -CheckoutRootPath $script:CheckoutRoot } "exactly 1.2.0-alpha.13"
    } })
    ...
    $checks.Add([pscustomobject]@{ Name = "fake checker stdout shapes are rejected by the exact PASS parser"; Run = {
      ...
      $hostileLines = @(
        ...
        @{ name = "relative directory"; stdout = "Show-ASIO local artifact PASS: target\show-asio-local\Syndocal_Show_ASIO_1.2.0-alpha.13_facefaceface_x64 files=3 distributionApproved=false"; stderr = ""; exit = 0; expect = "plain rooted Windows path" }
      )
      $allRejected = $true
    ...
    $checks.Add([pscustomobject]@{ Name = "checker-derived directory outside this checkout is rejected"; Run = {
      [void](New-ShowArtifactFixture)
      Install-ShowAuthoritySeams -StandardOut (New-PassLine "C:\foreign-checkout\target\show-asio-local\Syndocal_Show_ASIO_1.2.0-alpha.13_facefaceface_x64")
      try {
        $config = New-ShowConfiguration
