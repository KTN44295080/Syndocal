...
A temporary local diagnostic imported `C:\Users\kouty\Desktop\Shinkan-Left\Shinkan2026.dvc`, triggered the real `Shin` child Timeline, and observed natural playback at 9,958 ms. After the correction, channel 109 reached 255. The active `SS-Amber` targets initially rendered red/green/blue as 255/136/26, matching the imported 16-bit channel-section values 65535/34952/6682 after 8-bit conversion. A later standalone repeat capture made the corresponding Daslight live values legible as 255/135/25, not the initially misread 255/130/25, and identified the embedded single-beam feature values as the live source.

The direct-child seek path now anchors each Timeline fade to its authored occurrence time instead of restarting the fade at the moment of the seek. Overlapping fades with different start times are retained in one composite fade with per-target delay, duration, and Cue ownership. A permanent real-show golden seeks paused `Shin` to 9,950 ms and verifies channel 109 at 255 plus the four PinSpot RGB triplets at the currently verified live values 255/135/25. The same golden routes universe 0 to an independent ephemeral UDP receiver, parses the resulting ArtDMX packet, and requires all 512 packet bytes to equal the published paused-seek preview. The golden originally asserted the channel-section values 255/136/26 and was updated only after the repeated standalone Daslight capture established the live conversion below.

Daslight was then kept open with the same `Shinkan2026.dvc` project and its supported `DMX LEVELS` window. One window-state capture recorded the `Shin` scene card at `00m17s24` and Universe 1 values in the same observation (`target/qa/daslight-shin-timed-main.png` and `target/qa/daslight-shin-timed-dmx-levels.png`, both 2026-08-07 23:04:12 local). This removed the missing-time ambiguity. It also exposed that BPM-synchronised FX inside a Timeline were evaluated against Syndocal's global wall-clock beat phase, even while the child Timeline was paused at a fixed position. Repeating the same seek could therefore yield different bytes.

Timeline-owned FX now derive their beat counter and phase from the existing Timeline/RuntimeChildTransport activation anchor, including Scene Block source offsets and seek reconciliation. Live Cue-list FX retain the global clock. A regression seeks the same paused direct-child Chaser position at two different wall-clock instants and requires identical DMX frames; all 12 focused direct-child transport tests, the real-show Art-Net seek golden, and the full engine suite (427 passed, 2 manual tests ignored) pass.

Follow-up standalone captures closed the two identified semantic gaps. The importer now uses Daslight's embedded single-beam live feature values for unambiguous 8-bit RGB/dimmer channels and applies the observed live truncation rule: `SS-Amber` becomes 255/135/25, `SP-Aqua` 12/231/176, and `SP-Purple` 179/0/255. Multi-beam or ambiguous mappings retain the channel-section fallback. The `BB-Amber-Chaser` capture also showed 15–16 of 48 beams lit with `Size=32`; this establishes `Size` as a percentage of the selected strip rather than a beam count. The spatial evaluator now uses `size / 100`, with a 48-beam/32-percent regression requiring 16 lit beams at the sampled phase. Exact every-frame gradient and phase parity remains a separate byte-level acceptance boundary.
