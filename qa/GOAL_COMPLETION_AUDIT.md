...
## Addendum 2026-08-09: Visual MIDI/OSC Learn
...
- Current gates: `cargo test -p engine --locked` passed 445 software tests with two explicit physical/benchmark ignores; `cargo test -p io --locked` passed 98 software tests with one physical MIDI-port test ignored; full `cargo test -p syndocal --locked` passed 366 with nine explicit physical/capture/soak ignores, including DVC 40/40. Focused selected-Feature tests prove MIDI/OSC input, shared Engine batch output, and context-aware MIDI feedback (including mixed-value fail closed). The 27-assertion DVC MIDI source gate, 361-command backend operator contract, localization gate, and production web build are the final frontend gates for this tranche.
