...
| Lighting Cue Engine v2 | Complete locally; Daslight FX parity open | Cue Lists/Executors, tracking/block, Parts, IFCB Fade/Delay, Follow, MIB, HTP/LTP, Programmer/Blind, reference Palettes, ordered FX stack and seven-family/13-preset library; independent 2-8-stop Color engine with Cycle/Bounce/Sequence/seeded Random, RGB/HSV interpolation, deterministic spread, RGB/RGBW/CMY/HSV/colored-wheel bindings and publication-safe add/update; selective Effect ON/OFF Cue Recall with Effects Only/scoped Look update and published history boundaries; DMX Input/Merge, ENTTEC Pro-compatible/DMXKing/Open DMX serial output, RDM protocol tests | Per-Cue Effect parameter morph/fade and source-linked scene blocks; calibrated Amber/Lime/UV/multi-emitter colorimetry and physical color acceptance; independent chaser/move/curve/image-mapping engines; Art-Net/sACN/serial waveform, QLC+-class serial VID/PID and dual-port expansion, RDM fixtures/gateway, controller round trip |
