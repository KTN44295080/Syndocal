...
- The real `Codex-Chaser322-Probe.dvc` contains VALUE FX Rainbow ID621 with a three-stop grayscale palette and empty rack/PRESET beam sets. The importer now counts that exact source shape as Converted rather than Skipped, creates no runtime target and therefore adds no DMX output. A synthetic negative proves that a future targeted ID621 remains fail-closed until its feature/target save shape is observed. The previously mislabeled COLOR FX report family is corrected from `Colour Mappings` to `Colour FX`.
- At this VALUE FX tranche, focused synthetic and real-DVC tests passed, the complete DVC filter was 53/53, and the full locked all-target workspace was 1,180 passed / 13 existing hardware, GPU or long-duration ignores / zero failures. Production frontend build, localization 2990/2990, release metadata, DVC MIDI 39 assertions and DVC DMX 33 assertions passed. The newer Chaser PRESET addendum above records the current aggregate counts and release artifact.
- Remaining saved-DVC software evidence is explicit: a targeted VALUE FX specimen, any COLOR MAPPINGS specimen and a Move rack with `BEAMID>0`. Physical fixtures, MIDI feedback hardware, DMX node/electrical output, commercial visualizer/live LAN and Mac/Linux remain external acceptance boundaries.
