...
A temporary local diagnostic imported `C:\Users\kouty\Desktop\Shinkan-Left\Shinkan2026.dvc`, triggered the real `Shin` child Timeline, and observed natural playback at 9,958 ms. After the correction, channel 109 reached 255. The active `SS-Amber` targets rendered red/green/blue as 255/136/26, exactly matching the imported 16-bit source values 65535/34952/6682 after 8-bit conversion. The earlier inference that Daslight's 130/25 values were only a mid-fade sample was invalidated by the synchronized capture below; those bytes remain a real parity delta.

The direct-child seek path now anchors each Timeline fade to its authored occurrence time instead of restarting the fade at the moment of the seek. Overlapping fades with different start times are retained in one composite fade with per-target delay, duration, and Cue ownership. A permanent real-show golden seeks paused `Shin` to 9,950 ms and verifies channel 109 at 255 plus the four PinSpot RGB triplets at 255/136/26. The same golden routes universe 0 to an independent ephemeral UDP receiver, parses the resulting ArtDMX packet, and requires all 512 packet bytes to equal the published paused-seek preview. After this follow-up, the full engine suite passed 426 tests with 2 manual tests ignored, and all 6 local DVC golden tests passed.
