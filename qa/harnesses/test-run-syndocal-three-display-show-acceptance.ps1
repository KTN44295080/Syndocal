...
$script:GoodHash = "69E89678FFF38CD50F631BCA2E36D1FAFDB38FC0CB49F6C98254E715CC187178"
$script:WrongHash = ("cd" * 32)
$script:GoodSize = [uint64]62424576
$script:WrongSize = [uint64]61691905
$script:GoodVersion = "1.2.0-alpha.53"
$script:StaleVersion = "1.2.0-alpha.41"
$script:WrongVersion = "9.9.9-wrong"
$script:GoodArtifactHead = "27f45d1689f415a9423e431d1bf9bf285c0bd634"
$script:GoodHead = "27f45d1689f415a9423e431d1bf9bf285c0bd634"
...
function Invoke-FocusedChecks {
  ...
  try {
    ...
    $checks.Add([pscustomobject]@{ Name = "historical alpha.41 configuration is rejected"; Run = {
      Assert-Throws { New-ThreeDisplayConfiguration -IsApply $true -ExecutablePath $script:ExpectedPath -Sha256 $script:GoodHash -ProductVersion $script:StaleVersion -GitHead $script:GoodHead -EditorIdentity $script:EditorIdentity -LedIdentity $script:LedIdentity -ProjectorIdentity $script:ProjectorIdentity -LedId 41 -LedLabel "LED Program" -ProjectorId 42 -ProjectorLabel "Projector Program" -CdpPort 5189 -IntervalMs 200 -Attempts 3 -CheckoutRootPath $script:CheckoutRoot } "exactly 1.2.0-alpha.53"
    } })
    $checks.Add([pscustomobject]@{ Name = "wrong alpha.53 hash authority is rejected"; Run = {
      Assert-Throws { New-ThreeDisplayConfiguration -IsApply $true -ExecutablePath $script:ExpectedPath -Sha256 $script:WrongHash -ProductVersion $script:GoodVersion -GitHead $script:GoodHead -EditorIdentity $script:EditorIdentity -LedIdentity $script:LedIdentity -ProjectorIdentity $script:ProjectorIdentity -LedId 41 -LedLabel "LED Program" -ProjectorId 42 -ProjectorLabel "Projector Program" -CdpPort 5189 -IntervalMs 200 -Attempts 3 -CheckoutRootPath $script:CheckoutRoot } "exact 1.2.0-alpha.53 artifact authority"
    } })
    $checks.Add([pscustomobject]@{ Name = "current harness HEAD may equal alpha.53 artifact source HEAD"; Run = {
      ...
      New-Check -Passed $passed -Detail "current harness HEAD may equal the exact alpha.53 StandardRelease artifact source HEAD"
    } })
    ...
    $checks.Add([pscustomobject]@{ Name = "alpha.53 artifact source HEAD not ancestor is rejected"; Run = {
    ...
    $checks.Add([pscustomobject]@{ Name = "ShowAsioLocal does not inherit StandardRelease alpha.53 product lock"; Run = {
      ...
      New-Check -Passed $passed -Detail "ShowAsioLocal retains checker/manifest-bound product/version authority; StandardRelease alpha.53 metadata is not applied"
    } })
