...
Updated: 2026-08-27
Branch: `codex/syndocal-v1.2`; current development metadata is `1.2.0-alpha.17`. The DJ RELEASE tranche preserves the already-running Timeline position, relinquishes DJ clock ownership, and disables only the loop transport state; it passed engine DJ-Link 25/25, Tauri dispatch 8/8, first-party warning count 0, frontend build, detached release-metadata validation, and independent adversarial review with no P0. The standard alpha.17 native executable was built and locally verified as one responsive maximized window; its exact source and artifact identities are recorded in the QA handoff checkpoint. rb-output v1.1.8 and the target-DJ-PC physical HW-4 matrix remain pending, so no physical show acceptance is claimed. Historical alpha.16 and earlier checkpoints remain immutable evidence, and later cleanup or documentation commits do not redefine an already-built binary's source identity.
