...
| Authoring | General operator network and custom component model | Fixed Source → Transform → Output graph and one video effect slot per layer. The live quick rack reduces operator DOM/workload but does not expand graph semantics | TouchDesigner leads decisively |
