...
| DVC Super Scene timing | `RACK@GRID_BPM`をauthoring tempoとして復元し、audio解析`BLOCK@BPM`をproject clockへ誤用しない。非BPM親はblock ms、fade、signed source position、SPEEDを固定msで保持。`PLAY_TRIGGER=2`は`PLAY_DIVISION` beatsへchild全体をlive BPM駆動し、BPM変更時も位置を再アンカーする。Conform ONはsource rateを継承、OFFはwall-clock位相を維持する | manual v1.4 §4.3とDaslight 5.0.6.2のdispatcher/rate/block runtimeを照合。フルShinkanはShin=PLAY_TRIGGER 0 / 77 block、Unr=1 / 152 block、両方GRID_BPM 120 | 実ファイル229/229 blockはTimeline timing近似なし。BPM親のdirect/root child、pause/resume/seek、Conform ON/OFFのrate/phase、invalid division fallbackをengine/importer testで固定。監査Approximateは234→5 | 後続global BPM追従差は解消。残るTimeline timing境界はLoop Off時の動的source最終frame hold。詳細は`qa/DVC_SUPER_SCENE_TEMPO_PARITY.md` |
