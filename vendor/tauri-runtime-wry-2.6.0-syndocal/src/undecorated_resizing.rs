...
#[derive(Clone, Copy)]
struct HitTestBorder {
  x: WindowPositions,
  y: WindowPositions,
}

fn hit_test(bounds: HitTestBounds, point: HitTestPoint, border: HitTestBorder) -> HitTestResult {
  #[rustfmt::skip]
  let result = (LEFT * (point.x < bounds.left + border.x) as isize)
             | (RIGHT * (point.x >= bounds.right - border.x) as isize)
             | (TOP * (point.y < bounds.top + border.y) as isize)
             | (BOTTOM * (point.y >= bounds.bottom - border.y) as isize);
...
mod windows {
  ...
  use super::{hit_test, HitTestBorder, HitTestBounds, HitTestPoint, HitTestResult};
