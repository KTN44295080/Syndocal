...
Local visual comparison on 2026-07-13 used the installed Daslight 5 with `Shinkan2026.dvc` loaded. Its Control desk keeps group scene columns, the selected FX inspector, fixture-group tabs, the 2D stage and per-group Live Mixer visible together. Its Setup desk similarly keeps the fixture library, patch grid, group/stage map and dimmer/movement limits on one plane. The observed Chaser inspector exposes variant selection, direction, fading, pixels-on and random-flash controls; the official manual adds multiple feature ranges, fixture-index order, beam selection and step conversion. Syndocal's fixed full-window desks now match that task-density direction without copying Daslight's controls or colour system, but its lighting workflow still separates Live Edit, Timeline and several scene modifiers more than the loaded Daslight project does. The undocumented output semantics of Daslight's numbered Chaser variants are not assumed.
...
| Effects | Ordered LFO/Position Wave/Color/Chaser stack with reorder/bypass/clone/preset, BPM sync and graph-driven audio; independent Color provides 2-8 stops, RGB/HSV interpolation and RGB/RGBW/CMY/HSV/wheel output; independent Chaser provides ordered fixture/group/gap steps, 1-16 features, four directions, Wings, active pixels, duty/fading, Size/Phase/spread and free/beat clocks; selective Cue ON/OFF Recall supports Effects Only and scoped Look updates; 13-preset library is filterable as Colour, Chaser, Move, Value, Curve, Mappings and Colour Mappings | Seven live engines (Colour, Chaser, Move, Value, Curve, Mappings, Colour Mappings), multiple types per engine and unlimited stack | Colour, Chaser and Effect-state Cue recall are now independent software paths, but Daslight still leads overall: five Syndocal library families still share LFO/Position Wave, per-Cue parameter morph/fade, calibrated multi-emitter colour, editable curves and image/video mapping are absent, and practical stacking lacks full release/venue proof |
...
| Performance evidence | Two one-hour internal engine soaks, command/DMX latency telemetry, large-show tests at 200 fixtures/8 universes/100 cues, deterministic 200-fixture/64-effect regressions for both Color and Chaser, a maximum-control 10-effect/16-feature/64-fixture Chaser regression and virtualized 2,000-fixture/500-effect UI | No comparable official benchmark values are published | No fair winner; current Syndocal evidence does not cover mixed Color/Chaser release p95/p99/soak, 100+ live universes or same-host Daslight measurement |
...
2. Continue beyond the completed independent Color and Chaser engines and selective Cue Effect recall: add independent Move, Curve/Value and image/video Mapping runtimes, then add named racks, multi-select operations and release/venue evidence for practical stacking. Chaser's remaining gate is physical fixture/timing acceptance, not a shared LFO alias.
