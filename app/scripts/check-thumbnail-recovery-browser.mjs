...
try {
  ...
  for (const viewport of [{ width: 1280, height: 720 }, { width: 1920, height: 1080 }]) {
    ...
    await page.evaluate(() => window.__thumbnailRecoveryFixture.settle('asset', 0, false));
    ...
    await page.waitForFunction(() => window.__thumbnailRecoveryFixture.counts().asset === 2);
    assert.deepEqual(await page.evaluate(() => window.__thumbnailRecoveryFixture.counts()), { layer: 1, asset: 2 });
    await page.evaluate(() => { window.__thumbnailRecoveryFixture.settle('asset', 1, true); window.__thumbnailRecoveryFixture.settle('layer', 0, false); });
    ...
    await layer.click();
    await page.waitForFunction(() => window.__thumbnailRecoveryFixture.counts().layer === 2);
    assert.deepEqual(await page.evaluate(() => window.__thumbnailRecoveryFixture.counts()), { layer: 2, asset: 2 });
    await page.evaluate(() => window.__thumbnailRecoveryFixture.settle('layer', 1, true));
    ...
    results.push({ viewport, status: 'pass', retryButton: bounds,
      checks: ['no unauthorized reads', 'disabled repeat click', 'independent lanes', 'keyboard asset retry', 'layer retry', 'successful cache reuse', 'reset', 'live-only catalog', 'no native capability: disabled load and no dead retry'] });
    await page.close();
  }
