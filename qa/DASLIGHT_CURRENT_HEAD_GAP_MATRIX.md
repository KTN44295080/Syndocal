...
- 監査日: 2026-08-09
- 監査基準: 本ファイルを含む現HEAD（visual MIDI/OSC Learn + DVC MIDI shortcut復元 + Move Symmetry照合 + DVC beam-target保持）
...
| DVC MIDI shortcut import | `SHORTCUT TYPE=1`のEVENTをNote On/Off・CC・Program Changeへ構文検証。107=Scene Play、55=Tap Tempo、108/109/110=Forward/Reverse/Back & Forth、113=Bank Nextを復元する。各DVC BankはCue Listへ1:1保存し、113はTARGET SceneをList anchorとして保持。方向付きSceneは既存Cue起動へone-shot方向を渡し、pre-wait後も維持、hold解放時は既存`ReleaseCue`でauthored状態へ戻す | Homecoming実XML 17件、DSF実XML 2件、Laser実XML 2件を全数監査。107/55は既存Daslight UIとTouch action、108-113はDaslight実行ファイル内の連続action tableと実DVC TARGET/FLASHを照合 | Homecoming 17/17（13 Scene + Tap + 2 directional Scene + Bank Next）を復元。合成検体で未使用109=Reverseと非holdも固定。残る229は未推測で個別Unsupported | 入力device affinityとDaslight固有OUT/OUT1 LED velocity色はreport上Approximate。229（選択live fader）は選択feature/slot文脈の確定が次の対象 |
...
| DVC dynamic FX import | フルShinkanの30 effectをSkipped 0で変換。Bar-StrobeAMber=64 beams / 48 selections / BEAMID 0..7、Bar-Side Chaser=16 steps / 16 beams / BEAMID 0..7を保存・再読込・DMX隔離テストで固定。DVC MIDI shortcut追加後もDVC 39/39 PASS | 原XMLのBEAMS列とDaslightのgenerator/bodyを比較 | Curve/Chaserの旧fixture平坦化を解消、今回のmapping UI追加による非退行なし | VALUE FX / COLOR MAPPINGSはユーザーDVCに検体なし。MoveのBEAMID>0検体のみ未証明 |
