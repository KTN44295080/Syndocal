...
```json syndocal-q1-q4-ledger
{
  ...
  "q4_evidence": [
    ...
    ,{ "id": "EV-SHOWCLOCK-IPC-UI-2PROC-SW-2026-09-12", "scope": "Current-main ShowClock process-owned Primary/Standby Tauri IPC lifecycle, typed kind-bound action IPC, explicit local output arm, local lighting dispatcher and per-operation video ownership, fixed-capacity generation-bound scheduling, Manual Hold/Re-arm, STALE/FAULT permit revocation, exact-peer two-process sample loopback, stale-on-primary-loss, session-incarnation fence, Setup/IO UI wiring, status-driven action sequence recovery, and native release/process smoke", "kind": "current-source-automated", "status": "accepted-current", "date": "2026-09-12", "command": "MSVC 14.44.35207 x64; cargo test --manifest-path app/src-tauri/Cargo.toml --locked show_clock_ipc::tests -- --nocapture --test-threads=1; cargo test --manifest-path app/src-tauri/Cargo.toml --locked control_plane::tests -- --nocapture --test-threads=1; cargo test -p io --locked --test show_clock_two_process -- --nocapture --test-threads=1; pnpm.cmd --dir app exec tsc --noEmit; pnpm.cmd --dir app run build; pnpm.cmd --dir app run check:frontend-invokes; pnpm.cmd --dir app run check:frontend-command-routing; node app/scripts/check-tauri-admission-inventory.mjs; pnpm.cmd --dir app run check:output-control-runtime; pnpm.cmd --dir app tauri build --no-bundle; Exact target/release/syndocal.exe process smoke", "exit_code": 0, "assertion_count": 46, "ignored_count": 0, "warning_count": 0, "artifact_hash": "2E7F01BF5DD427E4544494B2303194D9A4ABA2E51EFCDAFD747ABAC1C4737C95", "artifact_na_reason": "The native release build produces the current checkout executable; this hash is the exact current-source process-smoke binary and is not a signed or published release artifact.", "raw_evidence_paths": ["qa/SHOWCLOCK_RUNTIME_LAN_REVALIDATION_2026-09-12.md", "qa/LUNA_SHOWCLOCK_REVIEW_HANDOFF_2026-09-12.md"], "currency_nonclaim": "Current-source ShowClock action/fence IPC/UI, software output arm and dispatcher, two-process loopback, and Windows native build/process-smoke evidence only; it does not constitute native UI interaction matrix, physical output, restart replay restoration, real wired two-machine partition/rejoin, witness/interlock, automatic failover, venue, signing, publication, or product completion.", "linked_q1_ids": ["COV-SHOWCLOCK-001"] }
