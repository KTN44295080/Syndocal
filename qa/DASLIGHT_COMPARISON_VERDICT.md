...
| 機能面 | 部分超過・部分劣位 | software監査PASS | **比較可能（全面同等ではない）** | 7独立FX、Cue transition、Scene Live/Live Mixer、GDTF onboarding、workspace/lock、Art-Net process境界に加え、DVC Homecoming MIDI shortcutを17/17復元（方向付きScene/Bank Next/hold解放を含む）。Laserのaction 229もCC8/9を選択中の表示Featureフェーダー1/2として復元し、UI/MIDI/OSC/Remote/AIを同じEngine batch経路へ統合した。一方、Daslight固有MIDI feedback色、商用catalog規模、standalone hardware、native mobile、実運用実績は未同等または未計測。 |
...
5. MIDI/OSC visual Learnのソフトウェア経路は実装・自動検証済み。実灯体、Art-Net node、物理MIDI/OSC controller、会場での手数・latency・feedback比較は未実施
