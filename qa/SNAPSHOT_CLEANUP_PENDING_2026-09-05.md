...
## 追加修正: 実アプリの二重preview取得と上部BPM編集

- branch/base: `codex/syndocal-v1.2` / `d06914d979e86bd335fcb507134339a71f240c49`。ユーザーの「まだかくつく」「上部BPMクリックで変更できない」を対象にした修正。前段の単体render benchmarkでは実UIの取得owner数を検出できていなかった。
- 原因: `MappingPersistentWorkspaceBand` がJSX-valued `contextContent` getterを表示と存在確認で二度評価し、不可視の `TimelineSourceShelf` / preview controllerも生成。表示位置の小さいcontext slotで生成結果をmemo化し、表示と判定が同じownerを使用する。setup/controlの外側mount境界、意図的なhidden child、contextなしのfallbackを維持。`TimelineSourceShelf`内のpreviewContent参照は一度のみ。
- 実native baseline: userが操作しているexact checkout PID30636をlocalhost CDPの読み取り専用observerで計測。UIクリック・seek・project変更は実施していない。`stutter-native-before-duplicate-fix.json` のretained 3000 requests / 約27秒は各output約55.5Hz、合計約111Hz（設計30/output、60total）、peak in-flight2、BUSY406/3000=13.53%。全応答ArrayBuffer。不可視ownerによる競合の根拠。ただしこの保存区間は全PTS0であり、再生中FPSやGPUdecode性能の根拠にはしない。先行再生中観測ではrender/paintより要求重複が目立ったが、保存された終端区間と混同しない。
- 上部BPMは従来drag-regionの表示spanで編集handlerがなかった。`TopbarBpmControl`へ分離し、クリック/ダブルクリック/ラベルクリックで数値入力、20〜300、Enter/blur確定、Escape取消、pending singleflight、編集中snapshotでdraftを上書きしない。既存ticketed `set_bpm`経由で手動BPM Undo契約を継承。拒否は入力と可視errorを保持し、同値確定時も古いerrorを消す。既存文字・hitサイズは維持。
- focused checks PASS: `pnpm --dir app exec tsc --noEmit`; `node app/scripts/check-workspace-context-content-browser.mjs`（実slot生成1回・context交換cleanup・hidden child切替・fallback・全owner破棄）; `node app/scripts/check-topbar-bpm-browser.mjs`（実component操作/拒否/同値error/二重commit防止）; `node app/scripts/check-timeline-output-monitors.mjs`; `node app/scripts/check-timeline-output-preview-browser.mjs`（1920/1280、画素/BUSY期限/ratio/canvas維持）。native buildと修正後測定は以下に追記する。
- context/BPMの独立レビューPASS。`node app/scripts/check-tauri-build-wrapper.mjs` PASS231assertions/27hostile mutations、`git diff --check` PASS。`pnpm --dir app tauri build --no-bundle` exit0、frontend7.42s/native2m14s、compiler/bundler first-party warnings baseline0/current0/delta0。exact checkout PID30636だけを事前停止、14.44.35207 linkerのabsolute pin/where-firstを確認。ログ `target/qa/snapshot-cleanup-20260905/context-bpm-native.log`。
- 修正後計測用native起動: PID43852、SHA256 `22F7666E32FDB6BB30E4A9A32555D992A872AEE59111304B74E8ECE0AAC9AF24`、Syndocal main window1件responsive/maximized。`launch-context-bpm-diagnostic.ps1` / `context-bpm-diagnostic-launch.json`。localhost38479 CDPは子process限定の一時計測設定、通常起動へ戻す。ユーザーの再生操作を待って計測する。BPM実nativeのユーザー操作受入は未確認。
- 修正後、ユーザーが同じ前景/背景を再生。`stutter-native-after-duplicate-fix-steady.json` は3000requests/約56秒、全PTS約65〜120秒、各output26.715Hz、同id10ms未満0件、peak in-flight1、BUSY0/3000。要求重複/競合の解消を実nativeで確認。render p95前景6.511ms/背景3.006ms、IPC往復p95 19.3/14.7ms、canvas putImageData p95 0.1ms、longtask0。同一PTS反復213/1499・231/1499、正のPTS差p95 67msは残る。前baselineは静止区間なのでrender/IPC値を同条件の速度比較には使わない。
- 二重生成修正の検証完了と、滑らかさの全面受入は別。残る候補は30Hz pollingのtimer遅延蓄積とengine/映像frame clockの位相・量子化。これらは本単位では変更しておらず、26.7要求/秒を26.7種類の映像frame/秒とも扱わない。次の安全な調査は再生時transport時刻と採用PTSの対応を記録し、要求cadenceとauthoritative clock側を分離する。ユーザーの体感改善とBPM操作のnative受入は回答待ち。
- observerを解除しexact PID43852のみ停止、計測envなしで同SHAの通常起動へ戻した。`launch-context-bpm-normal.ps1` / `context-bpm-normal-launch.json`: PID41140、Syndocal main1件responsive/maximized。localhost38479 Listenなしを確認。ユーザーprojectやnativeUIの自動操作なし。
- 本checkpoint所有8filesのみcommit/push。保護viewportcheckerのSHA256は既述値維持、未stageのまま。target/qaの計測・build・launch evidenceは保持しcleanupなし。この節のbaseとGit commitが修正単位を識別する。
