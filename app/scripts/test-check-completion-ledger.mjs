...
assert.deepEqual(result.statusCounts, { Open: 37, Deferred: 8, Complete: 13 });
...
expectFailure(
  ...
  (candidate) => { candidate.items.find((item) => item.id === "VIDEO-FULL-GATE-001").status = "Complete"; },
  /open source row VIDEO-FULL-GATE-001 must remain status Open/u,
);
...
expectFailure(
  ...
  (candidate) => { candidate.items.find((item) => item.id === "VIDEO-FULL-GATE-001").classification = "Deferred"; },
  /open source row VIDEO-FULL-GATE-001 must be Supported or External acceptance/u,
);
expectFailure(
  "wrong status for an open source row fails closed",
  (candidate) => { candidate.items.find((item) => item.id === "VIDEO-FULL-GATE-001").status = "Deferred"; },
  /open source row VIDEO-FULL-GATE-001 must remain status Open/u,
);
expectFailure(
  "wrong source kind fails closed",
  (candidate) => {
    const item = candidate.items.find((entry) => entry.id === "VIDEO-FULL-GATE-001");
    item.source.kind = "Deferred";
    item.classification = "Deferred";
    item.status = "Deferred";
  },
  /stale ledger ID VIDEO-FULL-GATE-001: source kind Deferred does not match Open/u,
);
expectFailure(
