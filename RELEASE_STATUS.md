...
大画面VJ operator面のLayer／Output密度を実show相当fixtureで更新した。Layerはcompact bankを3件から6件へ拡張し、各rowへBuilt-in FX、Enable／Bypass、Advancedを常設した一方、Import／Clear／説明／parameter editorはAdvancedを開いた対象だけmountする。Outputは全出力をLive／Off／BO付きrailへ常設し、rail／Prev／Next、選択detail、Program labelとProgram monitor request output IDを同じ選択へ同期する。7 layer（6+1 bank）、enabled／bypassed／noneを混在させたISF、3 outputの専用fixtureを英日×1920x1080／1366x768／1280x720で駆動し、Advanced DOM 0→1 editor・2 control→0、Bypassのtransaction／snapshot／ARIA、output 1→3の四者同期を通過した。初回のelement rect検査はfallback幅でAdvanced actionが約4px逸脱する実不具合を検出し、compact grid修正後は全4操作phaseでunsafe overflow 0／owner外rect 0となった。同じcurrent sourceの分離native ASIO QA targetを再buildし、空projectではあるが最大化work-area 1920x1032→F11 exact 1920x1080・重複headerなし→Esc 1920x1032復帰も再確認した。これはoperator UIとDOM budgetの合格であり、GPU multi-FX、4K長時間負荷、native pixel latency、SynapseRack／TouchDesigner同条件比較の代替ではない。受入境界は`qa/VJ_OPERATOR_DESK_ACCEPTANCE.md`を正とする。
...
ISF 2 single-pass image filterをレイヤー単位でimport、埋込保存、bypass、削除、parameter操作できるようにした。VJ Deskから即時適用できる14種の内蔵GPU FX（Invert、Monochrome、Threshold、Posterize、RGB Split、Mirror、Kaleidoscope、Zoom、Rotate、Strobe、Scanlines、Colorize、Vignette、Glitch Shift）をカテゴリ付きselectorで提供し、全shaderをproduction ISF準備経路で自動compileする。Event／Bool／Long／Float／Point2D／Colorを最大16 controlまで解釈し、GLSLをNagaで検証済みWGSLへ変換してwgpu上で実行する。変換結果とGPU pipelineは最大64 shaderまで再利用し、Preview、native display、NDI／Spout、recordingへ渡す共通frameに一度だけ適用する。不正shaderやGPU初期化失敗時は映像を止めず元frameへfail-openし、診断画面へ直近errorとpipeline数を出す。`.sdc v1`へshader sourceとcontrol値を埋め込み、旧layerはeffectなしで互換読込する。sourceは512KiB、image input 1個、single passに限定し、multipass／persistent buffer／imported resource／audio・audioFFT／loop／atomic／imageStore／明示binding・uniform・include・extensionを拒否する。実GPU threshold変換、cache再利用、fail-open、複製、control clamp、保存／旧file互換、不安全shader拒否を自動テスト済み。複数FX stack、100種以上のeffect breadth、実在ISF packの互換範囲と4K長時間負荷は競合到達／M4受入へ残る。
