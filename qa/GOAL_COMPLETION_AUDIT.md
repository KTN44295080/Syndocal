...
## Addendum 2026-08-09: Chaser PRESET feature and beam target restoration

- Chaser no longer assumes every saved effect targets Dimmer. The importer retains embedded-profile `SSLPRESETTYPE` values and resolves generic rack PRESET types 4/65/66/67 plus profile-selected type 81 to Dimmer/Red/Green/Blue/Amber. PRESET `MIN/MAX` is preserved as the authored feature range.
- `RACK/BEAMS` remains the authoritative target/selection list. `PRESET/BEAMS` is not substituted: Homecoming `strobe_wave-red` demonstrably retains two stale Strongpoint entries there while its Chaser rack targets four Saber Spots. Homecoming `saber_chase-red/green/blue` and `strobe_wave-red` now restore their RGB feature instead of Dimmer.
- Documents `Shinkan2026.dvc` scene `BackBar-Amber` resolves profile UID + channel 31 + preset 0 through `Amber8 / SSLPRESETTYPE=81`. Its 51 saved beams form 48 selections; 48 Mega Bar cells map to `ColorAmber` through `ColorAmber 8`, while three Mega Flash beams without Amber are omitted with an explicit approximation and without changing the selection sequence.
- The engine treats a concrete segment feature such as `ColorRed 2` as exactly one DMX attribute. It does not expand that Chaser target into the segment's Green/Blue channels or global Dimmer; only generic Dimmer retains the verified virtual-intensity behavior. Portable synthetic, actual Homecoming/Documents golden and engine isolation tests are green. The current DVC filter is 56/56; the locked all-target workspace is 1,184 passed / 13 existing hardware, GPU or long-duration ignores / zero failures. DVC MIDI 39 assertions, DVC DMX 33 assertions, localization 2990/2990 and release metadata also pass.
- `pnpm --dir app tauri build --no-bundle` passed after the exact-checkout process guard reported zero running instances. The resulting 2026-08-09 20:30:11+09:00 release executable is 38,995,456 bytes, version 1.1.0, SHA-256 `725ACD1B2ECE749579A77860C0BCB7DCED2E5505A7069FC4DA56544004366C3E`. It was intentionally not launched because another task owns the desktop surface; no current native-window verification is claimed.

## Addendum 2026-08-09: VALUE FX saved no-op and static catalog boundary

- The installed Daslight 5.0.6.2 binary (`25.0905.165.111`, SHA-256 `325D83EC54D41305D60B466B486EFE413B8F3E2656B45A544277C0CE9B87AE2A`) fixes the family pairs as VALUE FX `7/7`, MAPPINGS `6/8` and COLOR MAPPINGS `5/3`. Its generator registration table fixes all IDs/names listed in `qa/DVC3_EFFECT_MAPPING.md`; common named generators reuse the same factory across families.
- The real `Codex-Chaser322-Probe.dvc` contains VALUE FX Rainbow ID621 with a three-stop grayscale palette and empty rack/PRESET beam sets. The importer counts that exact source shape as Converted rather than Skipped, creates no runtime target and therefore adds no DMX output. Targeted VALUE now has an additive serialized spatial body and validates PRESET selector/range, RACK beam/selection order, grayscale palette and PARAM 3/10/11/12 before reusing the measured Color generator runtime. Concrete segment features drive only that attribute; generic Dimmer scales only the selected RGB segment. This targeted path is proven by a portable synthetic DVC because no targeted saved-DVC specimen has yet been found.
- Scene Settings no longer labels a one-click VALUE block while silently creating a Square LFO. It creates a real Value body with Black/White palette and Rainbow generator, reopens the Value editor, and exposes the complete Daslight VALUE catalog—Rainbow/Burst/Plasma/Knight Rider/Sweep/Sparkle/Random fill/Perlin—plus Syndocal's Custom Envelope. The same Solid interaction, including Sweep selection and its Direction Change switch, passed at 1920x1080, 1920x1032, 2048x1152, 1366x768 and 1280x720 with no app/document scroll or pane horizontal overflow.
- Daslight `CSweepEffect` was recovered from the same installed binary: constructor `0x140355D30`, vtable `0x140696440`, evaluator `0x1403665A0`, `Direction Change` property ID10/default false and one-row alternate 180-degree transitions. The common constructor `0x140350BE0`, effect serializer `0x140347160` and property serializer `0x14034E440` prove ID625's exact TYPE4/ID1 palette, TYPE6/ID3 Transform and TYPE2/ID10 Direction Change body. Syndocal matches the integer hard-boundary/current-next palette algorithm and now imports that exact schema, preserving no-op or targeted beam order while rejecting wrong TYPEs, extra PARAMs and unproven Transform=1. The evidence and deliberate 2D/real-golden boundaries are recorded in `qa/DVC_SWEEP_SOURCE_PARITY.md`.
- Current gates at this checkpoint: DVC 57/57 and focused VALUE 621/625 2/2 PASS; locked workspace 1,189 passed / 13 existing hardware, GPU or long-duration ignores / zero failures; DVC MIDI 39 assertions; DVC DMX 33 assertions; backend operator contract 364 commands / 204 literal frontend calls / 162 transactional mutations; localization 3033/3033; VALUE source contract and production frontend build PASS. The complete Solid -> Value -> Sweep -> Direction Change Scene Settings path passes at 1920x1080, 1920x1032, 2048x1152, 1366x768 and 1280x720 with zero app/document scroll.
- `pnpm --dir app tauri build --no-bundle` passed after the exact-checkout process guard reported zero running instances. The resulting 2026-08-09 22:53:22+09:00 release executable is 39,068,160 bytes, version 1.1.0, SHA-256 `B40955BCDD65581230B25D91238449799CEE3C9DB3B98D5FB5B1FB239763F95A`. Native Computer Use then found exactly one Syndocal window, retained the full-screen layout, changed Setup -> Control successfully and returned `ready=true` with the Timeline surface present.
- Remaining saved-DVC/software evidence is explicit: a real targeted VALUE FX specimen and real ID625 golden, any COLOR MAPPINGS saved specimen and a Move rack with `BEAMID>0`. ID625 Transform=1 and arbitrary 2D MAPPINGS Sweep orientation remain separate from the proven one-row VALUE path. Physical fixtures, MIDI feedback hardware, DMX node/electrical output, commercial visualizer/live LAN and Mac/Linux remain external acceptance boundaries.
