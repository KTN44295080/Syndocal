...
- **T24-A ステージ可読性 ✅完了 2026-07-29** — 41灯密度でのラベル判読不能を解消。詳細・監督実測は監査文書参照。
- T24-B Setup>Mapping上面の死空間解消（F2）＋タイムライン展開レーン高（F6）
- T24-C ファイルダイアログ親設定＋.dvcインポート進行表示（F3+F4、Rust側・.sdc/protocol変更なし）
- T24-D Super Scene導線（F5+F11: Matrixセルバッジ、タイムライン空状態ヒント、インポートレポートリンク）
- T24-E 小型摩擦バッチ（F7用語衝突/F9空きペイン/F10タップクリップ/F12復旧メッセージ英語）
