...
ownership remain fail-closed. Focused output-control checker, TypeScript,
Node syntax, and diff checks pass. The exact pinned Community MSVC
`14.44.35207` no-bundle native build passed in `3m34s`; the resulting
...
The current alpha.67 Engine also completed the bounded physical serial smoke
against the connected FTDI `COM3` device with the pinned Community MSVC 14.44
linker. `SYNDOCAL_PHYSICAL_MASTER=10` and
`SYNDOCAL_PHYSICAL_SECONDS=15` yielded
`physical_serial_rainbow_demo_drives_master_dimmer_and_rgb_cells` at
`1 passed / 0 failed` in `15.02s`, with `439` successful sends, no serial
send failure, and preview RGB `(54,73,255)`. This proves only the
Engine-to-COM write path; fixture light and downstream reception remain
physical checks.
...
correction. Setup → Lighting → Patch now counts fixture families in the
Verified, Bundled, and Cached / offline section headers instead of summing
their selectable modes. These are fixture-family counts; per-fixture Mode
counts remain in each fixture row.
The focused viewport contract follows the current merged Patch source browser
and no longer depends on the removed Library panel. Source parent is the
upstream-equal alpha.63 checkpoint `5503913be77a8bf665a886f4ff9c898165b20fa0`.

Source validation passes with first-party warnings `0`: the patch-profile
count checker (`10` assertions), fixture-catalog helper checker (`69`
assertions), all five Patch browser viewport sizes, TypeScript, Node syntax,
and scoped diff checks. The exact pinned Community MSVC `14.44.35207`
no-bundle native build passed in `3m26s` with first-party warnings `0` (only
...
window (`showCmd=3`). Physical USB-DMX, Art-Net, fixture, camera/MiraBox,
Spout/Unity, and audio endpoint rows remain external gates.
...
change, then admits exactly one present. Existing semantic output mutation
coverage still proves zero stale presents. Independent Terra xHigh rereview is
GO with P0/P1 `0`; its P2-only notes are a narrower explicit `position_ms`
assertion and the pre-existing final-token-load-to-GPU-call TOCTOU that needs a
...
route switching is therefore proven, but camera content is not. Latest UI
...
MiraBox/camera, and Unity/Spout remain unverified external gates. The alpha.57
native evidence remains historical. Display 1 changed foreground frames after
2.5s, proving animated presentation while Timeline ran. Timeline transport was
paused and restarted for the background sequence above while the main window
remained maximized at 1920x1032.
...
the verdict is `accepted` with `native_hardware_claim=false`. Performance,
USB-DMX, audible audio, camera content, and Unity/Spout remain separate gates.

## 2026-09-01 historical alpha.57 media-admission native authority

Alpha.57 was the preceding source candidate on `codex/syndocal-v1.2`, based on
...
Source checkpoint `1545bedd189c1f14b9656551ca5046cb1d02023c` is pushed and
upstream-equal. Immediately before build, only exact checkout alpha.56 PID
`54360` was terminated. The first attempt failed before output because
`FFMPEG_DIR` was unset; after explicitly supplying the existing FFmpeg 8.1.2,
LLVM, and ASIO SDK paths, the exact Community MSVC 14.44 no-bundle build passed
in `5m29s`, warnings `0`. Artifact ProductVersion/FileVersion is
`1.2.0-alpha.57`, size `62,310,912`, SHA-256
`ACB6A29A90F5D9CE5D57A406AC15488F870515B917B4654720F92332C86E3F2B`.
Exactly one checkout-owned PID `2440` exposes one responsive, visible,
maximized `Syndocal` window launched with the exact alpha53 local QA project;
...
that fix, and no retry was performed. USB-DMX, exact MiraBox, audible
PROGRAM/CUE, Unity/Spout, external DJ/pedal, and three-display pixel acceptance
