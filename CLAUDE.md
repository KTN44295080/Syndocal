...
- 2026-06-15 (Claude takeover, JSX-phase investigation — DECISION NEEDED, NOT YET ACTIONED): The first JSX-extraction candidate is the topbar/header + `workspaceTabs` + `setupModeTabs` block in `App.tsx` (around lines 9683-9760+ of the current file). A component for exactly this already exists — `app/src/components/WorkspaceChrome.tsx` — with a full `WorkspaceChromeProps` interface. HOWEVER `App.tsx` does NOT import or use it; it renders an inline copy instead, and the two have DIVERGED: `WorkspaceChrome` additionally renders a `controlModeTabs` nav (edit/live/mixer `ControlMode`) that the current `App.tsx` does not have (App.tsx has no `ControlMode`/`controlModes` at all). So `WorkspaceChrome.tsx` looks like an in-progress/abandoned refactor by the previous author that was never wired in. Wiring it as-is would ADD a control-mode sub-nav = a behavior change/regression, and the frontend has no UI test coverage to catch it. RECOMMENDATION before any further JSX extraction: decide the WorkspaceChrome direction with the user — either (a) introduce the edit/live/mixer control-mode concept into App.tsx and adopt WorkspaceChrome, or (b) trim `controlModeTabs` out of WorkspaceChrome to match current behavior and then wire it in as a pure dedup, or (c) delete the unused WorkspaceChrome.tsx if the control-mode idea is abandoned. Do NOT blind-wire it. This is the natural handoff point: module-level pure-helper extraction + duplicate cleanup are complete (App.tsx ~17,957 lines, down from ~19,200; 17 new helper modules; 7+ duplicate definitions consolidated; `App.tsx` top-level names now have ZERO overlap with sibling `app/src/*.ts` exports, verified by name-intersection check). All remaining reduction is JSX componentization inside `App()`, which needs design decisions like the WorkspaceChrome one above.
