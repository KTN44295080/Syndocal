...
const [controllerSource, controlSource, panelSource, fullLockOverlaySource] = await Promise.all([
  ...
  invoke: async (command, args) => {
    ...
    switch (command) {
      ...
      case "prepare_output_consent_v1": {
        ...
        prepareRequestId = request.request_id;
      ...
      case "query_output_consent_status_v1": {
        const request = args?.request;
        statusRequestId = request.request_id;
...
assert.equal(prepared.challenge.request_id, prepareRequestId);
assert.notEqual(prepared.execution_request_id, prepareRequestId);
assert.equal(await replyLossController.consentStatus(prepared), "ready");
assert.equal(statusRequestId > 0, true);
...
// A terminal receipt is accepted only when it is bound to the exact prepared
// fence and canonical argument fingerprint. Protocol mismatches are permanent
// failures and must never trigger an execution replay.
let mismatchedReceiptCalls = 0;
const mismatchedReceipt = runtime.createBlackoutReleaseRuntimeController({
  invoke: async (command, args) => {
    if (command !== "execute_output_control_v1") throw new Error("unexpected setup call");
...
// Canonical 128-bit base64url identity is stricter than a generic 22-character
// alphabet match. A non-canonical final sextet is rejected at the renderer.
...
// Production UI has one restricted four-command R4 dispatcher. The ordinary
// runtime panel no longer sends the legacy clear boolean, while Full Lock keeps
// release unavailable rather than turning the S0 engage control into a toggle.
...
assert.match(panelSource, /<BlackoutReleaseControl\s*\/>/);
