...
- Updated viewport acceptance so all four reference sizes require the dedicated VJ surface and reject a visible lighting desk in VJ mode. The maximized 1920x1080 result and screenshot are the visual sign-off; 2048x1152 is the extended ceiling and 1366x768/1280x720 are compact containment fallbacks.
- Added 14 GPU ISF presets (Invert, Monochrome, Threshold, Posterize, RGB Split, Mirror, Kaleidoscope, Zoom, Rotate, Strobe, Scanlines, Colorize, Vignette and Glitch Shift) to a categorized live selector. Every embedded shader is parsed and compiled by the production ISF preparation path in automated tests; viewport acceptance requires the selector on all four reference sizes.
