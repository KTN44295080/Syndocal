...
Version-state distinction (2026-08-26): the pushed HEAD is
`07a9b75f25e74e58739be251aad3b3f1e98986f7` and its
committed product metadata remains `1.2.0-alpha.11`. The dirty uncommitted
worktree synchronizes in-progress `1.2.0-alpha.12` metadata across the version
surfaces, and `pnpm --dir app run check:release` passes only while supporting
checker sources are present as untracked files; alpha.12 is therefore not yet a
reproducible checkpoint, tag, or artifact; because those checkers are
themselves untracked, that passing result is not reproducible from tracked
sources until they are committed. Native artifact state (measured 2026-08-26):
the on-disk `target/release/syndocal.exe` is 58,012,160 bytes, SHA-256
`42194E8AC7C2D9B07D40447AB509C4C926CC33AD83BC99E6D992DF04B03172BBF`,
ProductVersion/FileVersion `1.2.0-alpha.11`, LastWriteTimeUtc
2026-08-25T14:54:12Z. Its embedded alpha.11 version string does not establish
provenance: these bytes match no recorded accepted build, so the file is an
unprovenanced, unaccepted local rebuild. It is NOT accepted, must not be
launched or trusted, and no acceptance claim may cite it. The accepted
`1.2.0-alpha.11` artifacts remain separate historical identities only:
57,888,768-byte SHA-256
`1B010C40242A5C7DD7A2797EAC1ECA2D31BCACE4455BA57C7F935611075B582B`
(D4-checkpoint build, completion-flow section 37) and 57,888,768-byte SHA-256
`522074E96A235310C9D39D3200E2A9D9B5C68429F0160E92318B568A0FE5AEE2`
(main-checkout merge-parity build, completion-flow section 39); neither
matches the current disk bytes. No alpha.12 native binary exists. Acceptance
requires a fresh alpha.12 rebuild through the exact VS2022 14.44 linker gate
with newly recorded size/hash/version/window evidence.
...
| SHOW-P0-5 | DJ-Link survives setup, authenticated wired operation, disconnect, and app restart | Published v1.1.3 remains immutable but blocked by its `DJ_MASTER_CHANGED` mismatch. The sole current show route is controlled target-DJ-PC source: peer branch `beta-v1.1.2` at pushed HEAD `925880068500d42d71b2671fa8a99e5895aca4e2`, runtime checkpoint H `c6ebb0f`, source version `1.1.4`. It requires a checkout-external show JSON through `DJ_AGENT_CONFIG_PATH`; no-argument `start-all.bat` is the real launch and exact lowercase `--preflight-only` is the only alternate. Passing preflight is software-only and starts no show-side process. The real launch still requires the current real Syndocal token. Current topology is FOH `192.168.50.1` / DJ PC `192.168.50.2`, using only `syndocal-envelope-v2`; generic/v1 routes are retired. No installer or preflight result is promoted to hardware acceptance. | Both PCs run the pinned controlled source/native artifact with the real token, and HW-4.1 through HW-4.12 are recorded over wired LAN, including pedal, measured loop, dedupe, reconnect, restart, and concurrent Art-Net/sACN. | CONTROLLED V1.1.4 SOURCE PATH PINNED / REAL TOKEN + HARDWARE 0/12 OPEN |
| SHOW-P0-6 | The distributed artifact exactly matches the accepted source | Dirty-worktree product metadata is synchronized at `1.2.0-alpha.12` while pushed HEAD `07a9b75f25e74e58739be251aad3b3f1e98986f7` remains committed at `1.2.0-alpha.11`. `pnpm --dir app run check:release:self-test` passes only with supporting untracked checker sources present, so the result is not reproducible from tracked sources. The source is dirty, no alpha.12 binary exists, and the two accepted native artifacts remain historical alpha.11 identities. The current on-disk `target/release/syndocal.exe` is an unprovenanced, unaccepted local alpha.11 rebuild matching neither identity; it must not be launched or trusted. | Focused gates, artifact-wide warning ratchet, `check:release` from tracked sources alone, exact VS2022 14.44 linker, `pnpm --dir app tauri build --no-bundle`, exactly one responsive maximized exact-path window, evidence update, meaningful commit, and successful push. | ALPHA.12 METADATA DIRTY / NATIVE BUILD, COMMIT, AND PUSH OPEN |
| SHOW-P0-7 | ASIO is explicitly selected and remains truthful through start, live callback I/O, stop, fault, and restart | The isolated bridge and application loader source have clean-broken to ABI/schema v2 and one canonical `syndocal_asio_bridge.dll`; the old claim that the app loader remains ABI v1 is retired. Focused bridge/application configurations report zero first-party warnings, but the artifact-wide warning gate remains open. The exact 44.1 kHz / 2-channel i32 / 128-frame HOTONE Ampero bridge-only run completed for `3,600,031 ms` without promoting that bridge evidence to native operator acceptance. Formal matched 48 kHz ASIO/WASAPI, alpha.12 native load/UI/persistence/revalidation, occupied/reset/unplug/XRUN/no-callback recovery, restart, and latency remain open. `distribution_approved` remains false. The proposed non-default local-only `show-asio` feature/overlay is outside the normal MIT installer/updater route and has no accepted artifact yet. | The actual show driver/rate/channels/native format/buffer are persisted and revalidated; Start/Stop/Close, callback continuity, occupied/unplug/XRUN/no-callback recovery, restart, formal matched 48 kHz soak, and measured latency pass without WASAPI or another-driver substitution; the local-only artifact passes its own source/runtime manifest checker. | 44.1 KHZ BRIDGE-ONLY SOAK PASS / SHOW-ASIO ARTIFACT, ALPHA.12 NATIVE OPERATOR, MATCHED 48 KHZ, RECOVERY, LATENCY, AND DISTRIBUTION OPEN |
| SHOW-P0-8 | `人生オーバー` follows DJ-Link position/loop/pedal handoff and auto-transitions to `惑う星` | Absolute loop/release and authored Follow/BPM primitives exist, but the current path is not show-correct. MasterTrackActive always starts transport at zero and stores position/original BPM diagnostically; StateSync never converges position; a nonmatching nonempty content ID falls through to title/artist; the peer may send Active once before position/BPM exists and never retry; actual loop state is not routed; generic `Running` steals the pedal from DJ MIDI; and RELEASE is not session-fenced. Strict wire v2, required fresh position/effective BPM/session/deck identity, continuous revisioned Sync, measured loop state, atomic engine start/sync/release ownership, and late-event fencing are now the accepted clean-break contract. The controlled strict-v2 peer source path is pinned, but its real-token run and the exact authored Timelines/full physical sequence remain unexecuted. | Wired authenticated Master-track trigger, position follow, measured absolute 8/4/2-beat loop updates, pedal stop/band continuation, Timeline completion, and BPM-aware automatic `惑う星` transition pass end to end with duplicate/stale/disconnect/restart cases. | STRICT V2 CONTRACT + SOURCE PATH PINNED / REAL RUN, KDMX ENGINE CONTRACT, AND AUTHORING OPEN |
...
| FC-08 | DJ machine status/NIC errors become unavailable/empty UI, and legacy and generic wire envelopes remain accepted without a retirement gate. | Preserve typed failure reason; exact current envelope only; old wire unregistered after a bounded peer migration; wired restart evidence passes. | STRICT PEER WIRE V2 IMPLEMENTATION ACTIVE / KDMX OPEN |
...
Current authority correction (2026-08-26): pushed commit `7ee3b8f` tracks and
independently reviewed the exact cleanup safety harness; it is evidence of the
guard, not evidence of deletion. The current production Plan admits exactly one
candidate path, `target/debug/incremental`, and remained blocked by the dirty
worktree. Apply was not run, no path was deleted, and reclaimed bytes are
**0**. Every other path listed in the dated inventories below is currently
