...
## 2026-09-08 直接継続：thumbnail取消の描画経路への伝達

基準 `269c232`。詳細は [thumbnail描画取消](THUMBNAIL_RENDER_CANCELLATION_2026-09-08.md)。
前回のnative workerの取消フラグを、既存の描画・入力・デコードの取消経路へ
接続した。通常APIと取消APIは単一実装へ委譲し、待機枠の所有権、素材の
private copyと生成後のE/R/H検証を維持する。UI resetからnativeへの明示取消や
同期処理の強制中断・総停止期限は未実装であり、この変更の完了範囲に含めない。

追加6件と映像通常構成180件は成功。libav有効構成は191成功・2失敗・8 ignoredで、
全体成功とは扱わない。失敗2件は既存seek試験で、現行の1000ms catch-up範囲内の
前進をsession再生成と期待している。試験入力の修正命令がサービス側で拒否され、
未反映。decoder本体・閾値・厳密なピクセル/回数assertionは変更していない。
次はこの2件を、catch-upとseekの境界を両方保つ入力で修復して再実行する。
旧来の通常構成だけの成功記録をlibav全体成功に流用しない。

独立レビューではなく自己レビュー。Codex/Worksや別モデルへは委譲していない。
Mac検証7ファイルは別の未完了作業として保全し、製品全体の台帳状態は変更しない。

## 2026-09-08 直接継続：libav全体試験の復旧

基準 `ab8e670`。前節で残した2件の失敗を再現し、現行の1,000ms以内の
継続デコードと1,001ms以上の再生成を、実映像・PTS・画素・回数で区別する
テストへ修復した。最終libav有効構成は193成功・0失敗・8 ignored、
通常構成は180成功・0失敗・3 ignored。release、wrapper、台帳チェックも通過。
詳細は [libavシーク契約の検証](LIBAV_SEEK_CONTRACT_2026-09-08.md) を参照。

負の開始時刻のMPEG-TSでは直接シークが失敗し、既存の復旧経路が正しい
画素を返すことも確認した。開始時刻だけを補正する試案は別ケースのPTSを
ずらしたため全て戻し、最終製品コードは変更していない。この経路の効率化は
