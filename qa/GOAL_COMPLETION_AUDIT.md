...
- Scene Settings no longer labels a one-click VALUE block while silently creating a Square LFO. It creates a real Value body with Black/White palette and Rainbow generator, reopens the Value editor, and exposes Rainbow/Burst/Plasma/Knight Rider/Sparkle/Random fill/Perlin plus Syndocal's Custom Envelope. The same Solid interaction passed at 1920x1080, 1920x1032, 2048x1152, 1366x768 and 1280x720 with no app/document scroll or pane horizontal overflow. Daslight VALUE ID625 Sweep remains intentionally absent until its evaluator/parameter contract is recovered; the catalog name alone is insufficient evidence.
- Current gates: DVC 56/56; locked workspace 1,187 passed / 13 existing hardware, GPU or long-duration ignores / zero failures; DVC MIDI 39 assertions; DVC DMX 33 assertions; localization 3031/3031; VALUE source contract, five-size Scene Settings DOM path and production frontend build all PASS. `pnpm --dir app tauri build --no-bundle` passed after the exact-checkout process guard reported zero running instances. The 2026-08-09 21:47:18+09:00 release executable is 39,029,248 bytes, version 1.1.0, SHA-256 `0DC70EA32E7E6B8094539BA33563A9DBABA1995E772C5DFCD035E92459753139`. It was not launched because another task owns the desktop surface; current native-window verification is not claimed.
- Remaining saved-DVC/software evidence is explicit: VALUE Sweep semantics, a targeted VALUE FX saved specimen, any COLOR MAPPINGS saved specimen and a Move rack with `BEAMID>0`. Physical fixtures, MIDI feedback hardware, DMX node/electrical output, commercial visualizer/live LAN and Mac/Linux remain external acceptance boundaries.
