...
Updated: 2026-08-27
Branch: `codex/syndocal-v1.2`; current development metadata is `1.2.0-alpha.19`, based on exact upstream `e43edcf60dfa116d67bbb5ff096f4590df684964`. The alpha.19 Stage/Timeline source is uncommitted and its native artifact remains unaccepted until the recorded gates, build, launch, commit, and push complete. The preceding `1.2.0-alpha.18` at `db4eefc348b01ee05dd2dc87945afa85de8803e` remains the latest immutable native evidence checkpoint: exact MSVC 14.44 / locked full-workspace passed with `1164` pass / `0` fail / `11` intentional hardware-media ignores, zero first-party warnings, frontend build, DJ Link, `422` invokes, `3559` localization checks, viewport/setup harness pass, and exact-MSVC live discovery identifying Ethernet4 `192.168.50.1` as eligible. The clean release gate `4fc443d` passed after staging seven pinned DLLs. The active OneDrive checkout still requires a clean non-OneDrive rerun because `qa/FFMPEG_WINDOWS_RUNTIME_INVENTORY.json` is hard-linked to `C:\Users\kouty\Documents\.tmp.driveupload\867492`; the tracked file is unchanged and no alias deletion or replacement was performed. The alpha.18 artifact is `C:\Users\kouty\Documents\KDMX\target\release\syndocal.exe`, Product/FileVersion `1.2.0-alpha.18`, `58,740,224` bytes, SHA-256 `841068E08F80EB877FBA919FB86D3F52B3D4444314B47ABEF995BFA593E8D4F9`, LastWriteTimeUtc `2026-08-27T05:38:40.1840641Z`; its launch observed exactly one responsive process (PID `80264`, title `Syndocal`, window handle `854080`, `IsMaximized=true`, start `2026-08-27T14:38:57.8350806+09:00`). Native capture confirmed Web Remote/Security/Endpoints/DJ Link/Standby in the same disclosure stack and two wired candidates. The wired-binding refresh click remains unconfirmed because foreground PID retrieval failed; the five-viewport setup harness did confirm listener empty→count 0→Ethernet4 `192.168.50.1`→count 1, one invoke per phase, and disabled mutation controls. Target-DJ-PC deployment, token, LAN, reconnect, physical pedal, MIDI, and the physical HW-4 matrix remain pending, so no physical show acceptance is claimed. Historical alpha.17 and earlier checkpoints remain immutable evidence, and later source or documentation commits do not redefine an already-built binary's source identity.

## 2026-08-27 preceding alpha.18 checkpoint

This is the preceding immutable alpha.18 native build/launch/layout record,
