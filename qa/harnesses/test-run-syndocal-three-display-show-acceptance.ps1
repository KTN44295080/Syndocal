...
$script:GoodHash = "ACB6A29A90F5D9CE5D57A406AC15488F870515B917B4654720F92332C86E3F2B"
$script:WrongHash = ("cd" * 32)
$script:GoodSize = [uint64]62310912
$script:WrongSize = [uint64]61691905
$script:GoodVersion = "1.2.0-alpha.57"
$script:StaleVersion = "1.2.0-alpha.41"
$script:WrongVersion = "9.9.9-wrong"
$script:GoodArtifactHead = "1545bedd189c1f14b9656551ca5046cb1d02023c"
$script:GoodHead = "1545bedd189c1f14b9656551ca5046cb1d02023c"
...
function Invoke-FocusedChecks {
  ...
  try {
    ...
    $checks.Add([pscustomobject]@{ Name = "alpha.57 authority binds exact artifact metadata"; Run = {
    ...
    $checks.Add([pscustomobject]@{ Name = "historical alpha.41 configuration is rejected"; Run = {
      Assert-Throws { New-ThreeDisplayConfiguration -IsApply $true -ExecutablePath $script:ExpectedPath -Sha256 $script:GoodHash -ProductVersion $script:StaleVersion -GitHead $script:GoodHead -EditorIdentity $script:EditorIdentity -LedIdentity $script:LedIdentity -ProjectorIdentity $script:ProjectorIdentity -LedId 41 -LedLabel "LED Program" -ProjectorId 42 -ProjectorLabel "Projector Program" -CdpPort 5189 -IntervalMs 200 -Attempts 3 -CheckoutRootPath $script:CheckoutRoot } "exactly 1.2.0-alpha.57"
    } })
    $checks.Add([pscustomobject]@{ Name = "wrong alpha.57 hash authority is rejected"; Run = {
      Assert-Throws { New-ThreeDisplayConfiguration -IsApply $true -ExecutablePath $script:ExpectedPath -Sha256 $script:WrongHash -ProductVersion $script:GoodVersion -GitHead $script:GoodHead -EditorIdentity $script:EditorIdentity -LedIdentity $script:LedIdentity -ProjectorIdentity $script:ProjectorIdentity -LedId 41 -LedLabel "LED Program" -ProjectorId 42 -ProjectorLabel "Projector Program" -CdpPort 5189 -IntervalMs 200 -Attempts 3 -CheckoutRootPath $script:CheckoutRoot } "exact 1.2.0-alpha.57 artifact authority"
    } })
    $checks.Add([pscustomobject]@{ Name = "current harness HEAD may equal alpha.57 artifact source HEAD"; Run = {
      ...
      New-Check -Passed $passed -Detail "current harness HEAD may equal the exact alpha.57 StandardRelease artifact source HEAD"
    } })
    ...
    $checks.Add([pscustomobject]@{ Name = "alpha.57 artifact source HEAD not ancestor is rejected"; Run = {
    ...
    $checks.Add([pscustomobject]@{ Name = "ShowAsioLocal does not inherit StandardRelease alpha.57 product lock"; Run = {
      ...
      New-Check -Passed $passed -Detail "ShowAsioLocal retains checker/manifest-bound product/version authority; StandardRelease alpha.57 metadata is not applied"
    } })
