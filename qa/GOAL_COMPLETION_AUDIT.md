...
Updated: 2026-07-23 (T19-D re-audit; older addendum retained below)
Scope: Syndocal as a unified lighting and VJ application, preserving `.sdc v1` and the one-screen desk.

## Verdict

The software already has a broad, locally gated show-control core, but the declared Daslight-comparable lighting goal is not complete. T19-E calibrated multi-emitter colour, expanded Scene Live operation, fixture onboarding, detachable workspaces/operator lock and final native/Art-Net evidence remain. Windows cross-application NDI and Spout, a physical DirectShow camera, and physical MIDI port opening have host evidence; unavailable hardware acceptance remains explicitly external.
...
| Lighting Cue Engine v2 | Complete locally; Daslight FX parity open | Cue Lists/Executors, tracking/block, Parts, IFCB Fade/Delay, Follow, MIB, HTP/LTP, Programmer/Blind, reference Palettes, ordered FX stack and seven-family/13-preset library; independent Color, Chaser, Move, Value, Curve, fixture-order Mapping and embedded image/text/video 2D Colour Mapping runtimes/editors; per-Cue-owned Effect optional same-ID state fade with continuous/discrete output policy and video-target support; source-linked Scene Blocks resolve the latest Cue body with 1-256 loops, optional jump targets, legacy point compatibility and publication rollback; selective Effect ON/OFF Cue Recall and scoped Look update; DMX Input/Merge, ENTTEC Pro-compatible/DMXKing/Open DMX serial output and RDM protocol tests | Calibrated Amber/Lime/UV/multi-emitter colorimetry and physical Color/Chaser/Move/Curve/Mapping/Colour Mapping acceptance; Scene Block operator rehearsal with external timecode and a physical show rig; Art-Net/sACN/serial waveform, QLC+-class serial VID/PID and dual-port expansion, RDM fixtures/gateway, controller round trip |
...
| Performance/reliability | Complete for established local gates; representative-media and output-node venue soaks open | Three one-hour release soaks (base, live-FFT, mixed Color/Chaser/Move), zero drops/send failures, p99 timing budgets pass; release-build seven-family production stack at 200 fixtures × 64 effects measures p95 2.654 ms / p99 2.955 ms / max 3.460 ms under the unchanged 5/8/12 ms gate; deliberate all-64-transition stress measures p95 9.576 ms / p99 10.596 ms / max 10.878 ms under its separate 12/16/20 ms gate; 4K HAP R GPU and engine large-show tests pass | Representative multi-layer 4K media/mixed ISF-HAP one-hour venue-host soak and sustained physical Art-Net-node capture |
...
- `mixed_color_chaser_move_curve_mapping_release_stack_meets_44hz_budget`: release-build per-tick benchmark, 200 fixtures × 64 mixed effects across the current production stack, p95 2.654ms / p99 2.955ms / max 3.460ms on the 2026-07-23 T19-D run. Its deliberate all-64-transition companion measured p95 9.576ms / p99 10.596ms / max 10.878ms.
