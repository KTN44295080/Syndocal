...
function registeredNames(source) {
  ...
  const names = []; let pendingCfg = false, closed = false;
  // Only a standalone structural closing line can end the handler inventory.
  // Comments/strings containing a fake delimiter are rejected as invalid names.
  for (const line of remainder.split(/\r?\n/)) {
    const trimmed = line.trim();
    if (/^\](?:,|;|\(invoke\)|\))?$/.test(trimmed)) { closed = true; break; }
  ...
  assert.ok(closed, 'Missing standalone handler closing bracket');
...
for (const bad of [
  ...
  fixture([...names, '// ],', 'new_unreviewed_command']),
  fixture([...names, '/* ], */', 'new_unreviewed_command']),
  fixture([...names, 'known_command // ],', 'new_unreviewed_command']),
