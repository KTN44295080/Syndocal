...
if durable.exists():
    raise RuntimeError("R4 durable state module already exists")

durable.write_text(r'''//! Machine-local crash boundary for output-disruptive R4 request identity.
//!
...
impl DurableR4StateV1 {
    ...
    pub(crate) fn store_terminal(
        ...
        {
            self.safety_blackout_engaged = match receipt.outcome {
                protocol::control_plane_command::OutputControlReceiptOutcomeV1::Applied => false,
                protocol::control_plane_command::OutputControlReceiptOutcomeV1::NoOp => {
                    self.safety_blackout_engaged
                }
            };
