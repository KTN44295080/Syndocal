...
- Product name: Rayard.
- Developer / publisher: Seraf() KTN.
- Project files use `.ry`. Do not reintroduce `.kdmx` or KDMX naming.
...
- Video is currently a CPU/reference preview and state model path. Native wgpu video output, in-process FFmpeg/HAP, NDI, Spout, and Syphon transports are future-phase work.
...
- `app/src/components/`: extracted SolidJS UI panels/editors, including `DmxOutputConfigPanel.tsx`, `DmxRawMonitor.tsx`, `DmxTestFramePanel.tsx`, `DmxRoutesPanel.tsx`, and `EngineTelemetryPanel.tsx` for Phase 1 DMX verification/output diagnostics.
...
- Latest major checkpoint in this handoff: `cargo test --workspace`, `pnpm --dir app build`, and `pnpm --dir app tauri build --debug` passed on Windows.
...
1. Stabilize Phase 1 handoff:
   ...
   - Run a full workspace/Tauri build only at a milestone.
...
3. Phase 1 operational smoke:
   ...
   - Consider adding a deterministic sample `.fixture` or `.ry` under a `samples/` directory once the current project file format feels stable.
