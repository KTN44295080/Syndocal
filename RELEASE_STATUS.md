...
Node Graph SourceへAudio FFTを追加し、Timeline AnalysisまたはLive Input、Bass/Mid/High、gain、biasを選んで照明属性と映像parameterへ接続できるようにした。Timeline spectrum point間は線形補間し、解析dataなしは0、gain/bias後は0-1へclampする。Live InputはCPALのsystem defaultまたは列挙deviceからmono downmixし、Start時に確保する4本×2048 frameのfree／ready固定slot poolへ格納する。通常data callbackはheap allocation、mutex、FFTを行わず、異常に大きいcallbackも末尾8192 frameだけへ処理量を制限する。slot枯渇時は待機せず最古ready slotを置換して最新入力を優先し、callback回数、最大frame数、drop chunk／frame数、queue depthをatomic計測する。専用workerは再利用する1024 sample windowとFFT scratchでHann FFTを最大約30Hzで計算し、CPAL callback／capture時刻差とlocal worker到着時刻から現在／最大capture-age推定値を公開する。分割slotはframe offsetで補正し、250ms以上古いslotと非有限PCMを解析へ渡さない。RMS silence gate、0.65/0.35 smoothingと3-band meterを備え、44Hz engine threadでもFFTを実行しない。stream error／engine publish failure／250ms無入力は`stale`へfail-closedし、meterとsmoothingをzero、ephemeral spectrumを`None`へclearする。さらにEngine自身も最後のLive spectrumを250ms TTLで失効させるため、capture workerが停止しても値を無期限保持しない。worker panicはthread境界で捕捉して即座にstatusをstale化し、status pollも予期しない終了を検出してpending clearを再試行する。fault後のqueued chunkは再publishできず、queue full時もclearを再試行する。Stop時にclearが未受理ならruntimeとretry ownerを保持し、受理後だけstreamを破棄するため、旧runtimeのclearが新streamを消す世代交差も起こさない。UIはclear未受理を赤の`SAFETY CLEAR PENDING`、engine queue受理後をamberの`SAFETY CLEAR ACCEPTED`／`Live stale · clear accepted`と表示し、実出力適用ackなしにzero完了を断言しない。status pollは停止中もsingle-flightで続け、command実行中のpollを禁止し、renderer reload後の実runtime再発見、Stopとpoll応答の世代競合、status不明時の誤healthy表示を防ぐ。停止／project load時もspectrumをclearし、`.sdc`にはdevice状態を保存しない。旧Audio nodeはserde defaultでTimelineを維持する。Timeline補間DMX 128、Live Mid DMX 191、Some→NoneとEngine TTLでDMX zero、249/250ms watchdog、fault latch／re-arm、Stop中QueueFull retry、callback downmix／処理量上限／固定slot再利用／最古slot置換／timestamp offset補正／非有限値回復、FFT scratch再利用、silence/tone分離、legacy roundtripを自動テスト済み。engine適用ack、明示WASAPI buffer／sample rate／channel mix、ASIO、hot-plug自動復帰、onset/BPM、Audio Reactive RackとAuto VJは未達である。
