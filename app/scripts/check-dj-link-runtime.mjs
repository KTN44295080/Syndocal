...
assert.match(types, /titleContains/);
assert.match(types, /fallbackDeck/);
...
assert.match(panel, /value="title_contains">Title contains/);
assert.match(panel, /data-io-control="dj-track-selector-mode"/);
assert.match(panel, /data-io-control="dj-track-title-contains"/);
assert.match(panel, /data-io-control="dj-track-fallback-deck1"/);
assert.match(panel, /<label class="checkbox inlineCheckbox">[\s\S]*data-io-control="dj-track-fallback-deck1"/);
assert.match(panel, /Accept playing Deck 1 when no title matches/);
assert.match(panel, /mapping\.selector\.fallbackDeck === 1/);
assert.match(panel, /buildDjTrackSelector/);
assert.match(panel, /const normalized = normalizeDjTrackTriggerMappings\(next\);[\s\S]*if \(!normalized\.ok\) \{[\s\S]*setMappingError\(normalized\.error\);[\s\S]*return;[\s\S]*\}[\s\S]*props\.onDjTrackTriggers\(normalized\.value\);[\s\S]*resetMappingDraft\(\)/);
assert.match(app, /normalizeDjTrackTriggerMappings/);
