...
9. Extend the implemented allocation-free capture path and CPAL timestamp-derived age estimate with explicit WASAPI buffers/sample rates/channel mixes, feature-gated ASIO after the SDK/license decision, richer bands/onset/BPM, an Audio Reactive Rack and deterministic Auto VJ. The current three-band FFT is a useful shared lighting/video source, not TouchDesigner-class audio tooling or turnkey Auto VJ.
