...
| DVC Super Scene timing | `RACK@GRID_BPM`をauthoring tempoとして復元し、audio解析`BLOCK@BPM`をproject clockへ誤用しない。親がBPM駆動でない場合は`CONFORM_TO_TEMPO`が非作用なので、block ms、fade、signed source position、SPEEDを固定ms child transportで保持する | manual v1.4 §4.3はConformが親Super SceneのBPM driving時だけ作用すると定義。フルShinkanはShin=PLAY_TRIGGER 0 / 77 block、Unr=1 / 152 block、両方GRID_BPM 120 | 実ファイル229/229 blockはTimeline timing近似なし。監査Approximateは234→5 | BPM駆動親+Conformの後続global BPM追従と、Loop Off時の最終frame holdは検体付き実装まで明示境界。詳細は`qa/DVC_SUPER_SCENE_TEMPO_PARITY.md` |
