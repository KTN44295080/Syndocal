...
- **ハーネスのmeasure/evaluate本体はテンプレートリテラル**: 中に書く正規表現の`\d`等は
  クック時に`d`へ潰れる。必ず`\\d`と二重エスケープ（T5で実測: `/\d+/g`が数値に一切マッチせず
  断言が常時false化）。焦点実走の合否は**pass行でなくexit code**で確認する（カテゴリ判定は
  行頭pass/failに反映されないものがある）。
