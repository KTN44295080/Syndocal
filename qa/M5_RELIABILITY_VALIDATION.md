...
It injects 2,000 patched fixtures into the primary maximized 1920x1080 Stage Map workspace. The visible Mapping Fixture list reports `aria-rowcount=2000`, keeps no more than 20 fixture rows in the DOM at both the top and bottom of the scroll range, reaches `Large Fixture 2000`, and preserves full-window containment. Fixture lists use fixed-row windowing above their thresholds; Cue editing is bounded to 12 rows per page, while Clip Grid and video layers/outputs retain their existing banks/pages. The DMX Patch Grid is a separate fixed-bound surface: all 512 addresses are present in one 32x16 grid with one roving tab stop and bounded internal scrolling. `npm --prefix app run check:patch-viewport` verifies 1..512 uniqueness, no address-page selector, compact cell geometry, keyboard traversal through A512, high-address actionability, and zero outer scroll at all five browser viewports. Compact viewport pressure remains covered by the standard five-browser-viewport gate, including the 1920x1032 measured-work-area fixture, instead of making a small window the main large-show sign-off.
