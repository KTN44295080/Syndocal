...
The direct-child seek path now anchors each Timeline fade to its authored occurrence time instead of restarting the fade at the moment of the seek. Overlapping fades with different start times are retained in one composite fade with per-target delay, duration, and Cue ownership. A permanent real-show golden seeks paused `Shin` to 9,950 ms and verifies channel 109 at 255 plus the four PinSpot RGB triplets at 255/136/26. After this follow-up, the full engine suite passed 426 tests with 2 manual tests ignored, and all 6 local DVC golden tests passed.
