...
| Effects | Ordered LFO/Position Wave stack with reorder/bypass/clone/preset, shapes, phase/spatial direction, BPM sync, fixture/group/video targets and graph-driven audio; 13-preset quick-start library is filterable as Colour, Chaser, Move, Value, Curve, Mappings and Colour Mappings | Seven live engines (Colour, Chaser, Move, Value, Curve, Mappings, Colour Mappings), multiple types per engine and unlimited stack | Discoverability and common one-click looks improved, but Daslight still leads materially: Syndocal's seven labels currently share two runtime primitives and do not yet implement multi-colour or image/video mapping engines |
...
- Replaced the unclassified effect samples with an accessible seven-family rail and 13 preset cards. Added Curve Saw, target-required Colour Spectrum and target-required spatial Colour Chase. Browser acceptance checks All/Colour filtering at all reference sizes; `qa/LIGHTING_FX_ACCEPTANCE.md` records the explicit non-parity boundary.
