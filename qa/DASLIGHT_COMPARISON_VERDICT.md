...
| 機能面 | 部分超過・部分劣位 | software監査PASS | **比較可能（全面同等ではない）** | 7独立FX、Cue transition、Scene Live/Live Mixer、GDTF onboarding、workspace/lock、Art-Net process境界に加え、DVCの検証済みMIDI Scene/Tap/Flash shortcutを復元。一方、DVC action 108/110/113/229、Daslight固有MIDI feedback色、商用catalog規模、standalone hardware、native mobile、実運用実績は未同等または未計測。 |
