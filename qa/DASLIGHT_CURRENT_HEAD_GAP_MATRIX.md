...
| DVC MIDI shortcut import | `SHORTCUT TYPE=1`のEVENTをNote On/Off・CC・Program Changeへ構文検証し、実検体で確定した107=Scene Play、55=Tap Tempoを復元。`FLASH=1`は専用FlashCueとしてpressでTrigger / releaseで既存Engine `ReleaseCue`を通す。DVC import後に旧割当を消すだけでなく、復元割当をproject-local mappingへ設置して`.sdc`保存へ引き継ぐ | Homecoming実XML 17件、DSF実XML 2件、Laser実XML 2件を全数監査。107/55の意味は既存Daslight UI照合とTouch actionの同一コードで二重確認 | 既知14件（Homecoming 13 scene + 1 Tap）を復元。108/110/113/229は意味を推測せずUnsupportedへ個別表示 | 入力device affinityとDaslight固有OUT/OUT1 LED velocity色はreport上Approximate。229（選択live fader）と108/110/113は次の保存差分/実画面照合対象 |
