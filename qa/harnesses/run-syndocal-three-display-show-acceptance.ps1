...
$script:ThreeDisplayRequiredProductVersion = "1.2.0-alpha.16"
...
function Test-ThreeDisplayCheckoutClean {
  ...
  if ($entries.Count -ne 0) {
    throw "Fail closed: exact alpha.16 artifact acceptance requires a clean checkout; git status reported $($entries.Count) change(s)."
  }
  return $true
}
...
function New-ThreeDisplayConfiguration {
  ...
  if ($IsApply -and -not $fullyConfigured) {
    throw "Fail closed: -Apply requires exact alpha.16 hash/version/HEAD, all three stable monitor identities, both explicit live output IDs/labels, and one explicit loopback CDP port."
  }
  if ($fullyConfigured) {
