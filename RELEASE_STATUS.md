...
Node Graph SourceへAudio FFTを追加し、Timeline AnalysisまたはLive Input、Bass/Mid/High、gain、biasを選んで照明属性と映像parameterへ接続できるようにした。Timeline spectrum point間は線形補間し、解析dataなしは0、gain/bias後は0-1へclampする。Live InputはStart時に確保する4本×2048 frameのfree／ready固定slot poolへmono downmixし、通常data callbackではheap allocation、mutex、FFTを行わない。異常に大きいcallbackも末尾8192 frameへ処理量を制限し、slot枯渇時は待機せず最古ready slotを置換して最新入力を優先する。I8／I16／I24／I32／I64／U8／U16／U32／U64／F32／F64の全CPAL PCM sample formatを同じ固定poolへ正規化し、callback回数、frame current／min／max、drop chunk／frame、queue current／high-water／capacityをatomic計測する。専用workerは再利用する1024 sample windowとFFT scratchでHann FFTを最大約30Hzで計算し、CPAL callback／capture時刻差とlocal worker到着時刻から現在／最大capture-to-worker推定値を公開する。分割slotはframe offsetで補正し、250ms以上古いslotと非有限PCMを解析へ渡さない。RMS silence gate、0.65/0.35 smoothingと3-band meterを備え、44Hz engine threadでもFFTを実行しない。

WindowsのLive InputはCPAL既定hostのWASAPI shared modeで、system defaultまたは列挙device、sample rate、requested fixed buffer、全channel平均／単一channel／stereo pairのdownmixを明示選択できる。device catalogは更新世代ごとのopaque IDと重複名を区別するlabelを返し、古い／未知IDはsystem defaultや同名deviceへ黙ってfallbackせず拒否する。per-device capabilityからdefault configと対応channel／sample-rate／sample-format／buffer rangeを取得し、非対応構成、8192 frame超または200ms超のfixed bufferをStart前に拒否する。64pxのfull-window railは停止中にdevice／rate／requested buffer／mix、live中にoverrun／capture-to-worker／選択I/Oと実callback frame／queueを表示する。B/M/Hは安全状態だけを返す軽量commandで約30Hz、詳細statusは約1Hz、device Refreshと各pollはsingle-flightとし、古いcapability応答を無効化する。1280x720と1920x1080のstopped／worst-case live telemetry viewport gateは、rail重複なし、3 meter、4 telemetry badge、実Clip Grid scrollport内のpad可視性とcritical overflowなしを通過した。Windows nativeでは最大化したcurrent-source buildでsystem default WASAPI shared入力を48kHz monoでStart／Stopし、callback current／min／max 480／480／480 frames、capture-to-workerの採取時点current 10.0ms／max 10.2ms、OVR 0／0、queue current／high-water／capacity 0／1／4を確認した。この1構成を長時間、ASIO、全hardwareの性能証明には読み替えない。
...
stream error／engine publish failure／250ms無入力は`stale`へfail-closedし、meterとsmoothingをzero、ephemeral spectrumを`None`へclearする。Engine自身も最後のLive spectrumを250ms TTLで失効させる。worker panicはthread境界で捕捉し、status pollも予期しない終了を検出してpending clearを再試行する。fault後のqueued chunkは再publishできず、queue full時もclearを再試行する。Stop時にclearが未受理ならruntimeとretry ownerを保持し、受理後だけstreamを破棄するため、旧runtimeのclearが新streamを消す世代交差を防ぐ。UIは未受理を赤の`SAFETY CLEAR PENDING`、engine queue受理後をamberの`SAFETY CLEAR ACCEPTED`／`Live stale · clear accepted`と表示し、実出力適用ackなしにzero完了を断言しない。停止／project load時もspectrumをclearし、`.sdc`にはdevice状態を保存しない。Timeline補間DMX 128、Live Mid DMX 191、Some→NoneとEngine TTLでDMX zero、249/250ms watchdog、fault latch／re-arm、Stop中QueueFull retry、callback downmix／処理量上限／固定slot再利用／最古slot置換／timestamp offset補正／非有限値回復、FFT scratch再利用、silence/tone分離、legacy roundtripを自動テスト済み。engine適用ack、同一device hot-plug自動復帰、ASIO、capture-to-analysis／engine／pixel latency percentile、8／16 band、onset／live BPM、Audio Reactive Rackと決定的Auto VJは未達であり、TouchDesigner同等をまだ主張しない。
