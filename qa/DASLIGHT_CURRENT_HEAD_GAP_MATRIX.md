...
- 監査日: 2026-08-09
- 監査基準: 本ファイルを含む現HEAD（visual MIDI/OSC Learn + Move Symmetry照合 + DVC beam-target保持）
...
| MIDI / OSC visual Learn | 共通トップバーのMIDI/OSC Learnを押すと対応controlが紫色になり、対象clickで点線選択、次のMIDI/OSC入力を即割当して監視を自動開始。既存source競合は置換し、timeout/error時は直前listenerを復元。詳細editor、`.midimap` / `.oscmap`、user templateを維持 | **2026-08-09最大化Daslight参照**: MIDI Mapping有効時に対象controlが紫色、選択targetが点線表示、次入力で関連付く操作モデル | MIDIは同じvisual Learn手順、OSCにも同じ手順を拡張 | 5解像度で42pxトップ列・32x40 Learn button・Cue 301への実mapping生成を固定。物理controllerの手数/latency/feedbackは外部受入 |
...
| DVC dynamic FX import | フルShinkanの30 effectをSkipped 0で変換。Bar-StrobeAMber=64 beams / 48 selections / BEAMID 0..7、Bar-Side Chaser=16 steps / 16 beams / BEAMID 0..7を保存・再読込・DMX隔離テストで固定。visual Learn追加後もDVC 37/37 PASS | 原XMLのBEAMS列とDaslightのgenerator/bodyを比較 | Curve/Chaserの旧fixture平坦化を解消、今回のmapping UI追加による非退行なし | VALUE FX / COLOR MAPPINGSはユーザーDVCに検体なし。MoveのBEAMID>0検体のみ未証明 |
