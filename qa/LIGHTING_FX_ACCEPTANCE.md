...
The design sign-off viewport is a maximized 1920x1080 application window. Information hierarchy, editor density, pane balance and screenshots are judged there first. This Windows workstation's measured maximized client area, 1920x1032, is a second required native-window gate. The 2048x1152 run is an extended-width ceiling regression. The 1366x768 and 1280x720 runs are mandatory compact fallbacks that prove containment and reachable pane-local scrolling; they are not substitutes for either maximized review and must not be used alone to call a desk well designed.

`check:viewport` starts its headless browser at 1920x1080 and runs the 1920x1080 primary gate, 1920x1032 measured-client-size fixture, 2048x1152 ceiling, 1366x768 fallback and 1280x720 fallback in that order. The 1920x1032 run remains a browser fixture and is not native evidence. When `SYNDOCAL_VIEWPORT_SCREENSHOT_DIR` is set, the default evidence set captures both large browser fixtures; `SYNDOCAL_VIEWPORT_CAPTURE_ALL=1` is an explicit diagnostic override for compact/ceiling screenshots.
