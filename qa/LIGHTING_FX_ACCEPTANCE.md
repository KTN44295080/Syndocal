...
The design sign-off viewport is a maximized 1920x1080 application window. Information hierarchy, editor density, pane balance and screenshots are judged there first. The 2048x1152 run is an extended-width ceiling regression. The 1366x768 and 1280x720 runs are mandatory compact fallbacks that prove containment and reachable pane-local scrolling; they are not substitutes for the 1920x1080 visual review and must not be used alone to call a desk well designed.

`check:viewport` runs in that order and starts its headless browser at 1920x1080. When `SYNDOCAL_VIEWPORT_SCREENSHOT_DIR` is set, the default evidence set captures the 1920x1080 primary viewport only; `SYNDOCAL_VIEWPORT_CAPTURE_ALL=1` is an explicit diagnostic override for compact/ceiling screenshots.

The production Type control now exposes `Move`, and the complete Move editor viewport gate passed on 2026-07-13. At all four viewports it requires the dedicated path desk, inspector, canvas and point operations to remain horizontally contained, with both the outer Inspector form and point list able to reveal their last controls. At 1920x1080 it additionally requires a real two-column path/inspector layout, a minimum useful canvas, and at least 98% surface/column area coverage with no measurable unused right or bottom strip. Compact fallback stacking and internal scrolling are allowed; a one-column 1920x1080 result remains a failure.
