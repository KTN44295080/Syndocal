...
impl WindowEventWrapper {
  fn map_from_tao(event: &TaoWindowEvent<'_>, _window: &WindowWrapper) -> Self {
    let event = match event {
      ...
      TaoWindowEvent::Focused(focused) => {
        ...
        #[cfg(windows)]
        if _window.has_children.load(Ordering::Relaxed) {
          const FOCUSED_WEBVIEW_MARKER: &str = "__tauriWindow?";
          let mut focused_webview = _window.focused_webview.lock().unwrap();
...
impl WindowBuilder for WindowBuilderWrapper {
  ...
  fn shadow(self, enable: bool) -> Self {
    ...
    {
      let mut this = self;
      this.inner = this.inner.with_undecorated_shadow(enable);
      this
    }
    #[cfg(target_os = "macos")]
    {
      let mut this = self;
      this.inner = this.inner.with_has_shadow(enable);
      this
    }
...
pub struct Wry<T: UserEvent> {
  context: Context<T>,
  // This strong reference keeps tao's Rc-backed target on the main thread.
  // Keep it declared before `event_loop` so it is dropped first (upstream
  // tauri#15411 commit 9a99265).
...
impl<T: UserEvent> RuntimeHandle<T> for WryHandle<T> {
  ...
  /// The raw display handle is obtained on the main thread. The returned raw
  /// handle retains the legacy RuntimeHandle lifetime contract; on Windows the
  /// display handle is pointer-free. See tauri-apps/tauri#15411.
  fn display_handle(
    &self,
  ) -> std::result::Result<DisplayHandle<'_>, raw_window_handle::HandleError> {
    get_raw_display_handle(self)
      .map_err(|_| raw_window_handle::HandleError::Unavailable)?
      .map(|handle| unsafe { raw_window_handle::DisplayHandle::borrow_raw(handle.0) })
  }
  ...
  match message {
    ...
    Message::EventLoopWindowTarget(message) => match message {
      ...
      EventLoopWindowTargetMessage::SetTheme(theme) => {
        event_loop.set_theme(match theme {
          Some(Theme::Light) => Some(TaoTheme::Light),
          Some(Theme::Dark) => Some(TaoTheme::Dark),
          _ => None,
        });
      }
      EventLoopWindowTargetMessage::RawDisplayHandle(sender) => sender
        .send(
          event_loop
            .display_handle()
...
fn create_webview<T: UserEvent>(
  ...
  context: &Context<T>,
  pending: PendingWebview<T, Wry<T>>,
  _focused_webview: Arc<Mutex<Option<String>>>,
  ...
  #[cfg(windows)]
  {
    ...
    unsafe {
      ...
      let focused_webview_ = _focused_webview.clone();
