...
| Move | Fan, Circle | Static Position Wave fan; independent paired Pan/Tilt Move circle |
...
- Browser acceptance passed with 1920x1080 as the primary operational/visual gate, 1920x1032 as the measured-work-area browser fixture, 1280x720 plus 1366x768 as containment fallbacks and 2048x1152 as the extended ceiling. It requires the three-pane desk, both fixed action docks, Stack/Graphs switching, 8 family buttons, 13 total cards, 4 target-required cards and the filtered Colour view. It also switches to Multi-color, adds/removes a stop, changes Random/HSV-longest, verifies the gradient and asserts that scalar Attribute/waveform/video controls are absent with zero horizontal overflow. The target/draft/status summaries identify the current whole-fixture Color edit and do not retain stale LFO/Position Wave text;
...
The Color, Chaser and Move tranches are real independent engines, not single-attribute aliases. Four library families still share the existing LFO/Position Wave primitives, so the full library must **not** be described as Daslight 5 FX parity.
