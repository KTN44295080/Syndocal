...
The implementable software-only scope is feature-complete and locally gated. It is not honest to call the product universally perfect until the external acceptance rows below are captured with the target show hardware, third-party VJ applications, representative media, two show computers, and release signing credentials.
...
| Time and output continuity | Complete locally | Shared clock source, MTC/LTC/Link timeline sync, LOCK/STALE age display, DMX route exponential reconnect telemetry/tests | Physical MIDI/LTC source and DMX node/interface disconnect capture |
...
| VJ workflow | Complete on Windows software paths | Media import, Clip Grid/Deck A-B, Preview/Program, audio/FFT, camera/screen workers, recording, NDI/Spout, ISF safe subset, HAP R 4K GPU, projection blend/mask | External OBS/Resolume/TouchDesigner tests; real camera; encoded HAP/ISF packs; projector rig; long A/V drift |
