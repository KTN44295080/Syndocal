...
The default-feature `syndocal` focused tests and the SDK-free ownership seams passed. The NDI-only io check/test and syndocal feature test were attempted but are blocked in this environment: the NDI SDK header is absent at `C:\TEMP\NDI 6 SDK/include`, and the feature test executable cannot load the NDI runtime DLL (`STATUS_DLL_NOT_FOUND`). No NDI or Spout SDK/hardware behavior is claimed from those seams. No combined all-feature check, package installation, native release build, or native UI launch was performed.

## Deliberate verification boundaries

No native release build, native UI launch, process control, hardware output, Daslight interaction, LAN peer testing, or distributed ownership test was performed. The final native release build and responsive-window verification are supervisor-owned under the repository completion gate.
