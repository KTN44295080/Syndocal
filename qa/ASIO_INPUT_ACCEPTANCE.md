...
- [x] FC-09 source/browser selection-loss handling is fail-closed at commit `a65e3ccb4a8dd14f4941109d06e1644876329a8c`: backend/device catalogue failure retains the exact unavailable stable identity, performs no WASAPI/first-device substitution, downgrades saved readiness, blocks passive same-identity recovery, and permits only explicit exact current-device reselection. The dedicated EN/JA browser contract proves cold-first-failure, passive new-ID recovery, forged stale input, unchanged persisted bytes, and zero control/Start IPC while locked. This is not current native DLL or physical-driver acceptance.
