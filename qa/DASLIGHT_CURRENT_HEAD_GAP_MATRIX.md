...
| DVC Super Scene timing | `RACK@GRID_BPM`をauthoring tempoとして復元し、audio解析`BLOCK@BPM`をproject clockへ誤用しない。非BPM親はblock ms、fade、signed source position、SPEEDを固定msで保持。`PLAY_TRIGGER=2`は`PLAY_DIVISION` beatsへchild全体をlive BPM駆動し、BPM変更時も位置を再アンカーする。Conform ONはsource rateを継承、OFFはwall-clock位相を維持。`ALLOWLOOP`はONで周期反復、OFFでFX/Step最終frameをblock終端まで保持する | manual v1.4 §4.3とDaslight 5.0.6.2のdispatcher/rate/block runtimeを照合。フルShinkanはShin=PLAY_TRIGGER 0 / 77 block、Unr=1 / 152 block、両方GRID_BPM 120 / 全229 block Loop On | 実ファイル229/229 blockはTimeline timing近似なし。BPM親のdirect/root child、pause/resume/seek、Conform ON/OFFのrate/phase、invalid division fallback、Loop ON/OFFの周期/holdをengine/importer testで固定。監査Approximateは234→5 | 現在検証済みDVC Scene Block timing fieldに既知のsoftware差なし。depth-1を超えるnested Timeline参照は別の構造境界。詳細は`qa/DVC_SUPER_SCENE_TEMPO_PARITY.md` |
