...
|---|---|---|---|---|
| UI見た目のリッチさ | 未計測 | 未計測 | **未計測** | 構造要素（identity色、ブロック解剖、固定インスペクタ、密度）は同等圏に到達したことをネイティブ実機で確認。ただし Daslight 側は実ショー41灯で埋まり、Syndocal 側は mini-show 1灯のため、同一コンテンツでの並置が成立していない。質感・階層の同等/超過は判定保留。 |
| 操作性 | 未計測〜劣 | 劣 | **劣（暫定）** | 計測済み3タスクで勝ちなし。シーンのタイムライン配置が 2操作 vs Daslight 1操作で劣位（計測当時）。T4 のダブルクリック直接配置で改善した可能性があるが、操作手数ハーネス `check-operation-counts.mjs` が旧UIセレクタ（`.timelineDeskTabs` の 'Cues'）のまま現行UIに対して破損しており再計測未完。計測が更新されるまで劣判定を維持する。 |
...
3. FXジェネレータの独立エンジン幅（Curve / Mappings / Colour Mappings 相当）とプリセット量で劣る
4. シーン単位 Live modifier 群（speed / phase / size / direction / segment / flash / strobe / solo の即時操作）が不足
5. シーン配置の操作手数で計測劣位（2 vs 1、再計測未完）
...
1. `check-operation-counts.mjs` を現行UI（T2/T4/T10 後）へ修復し再計測 — 特に T4 ダブルクリック配置で「シーン配置」が 1操作化したかを確認
2. `.dvc` インポータ（`qa/DVC_IMPORT_FEASIBILITY.md`）で Shinkan2026 を Syndocal に読み込み、同一ショー・同一解像度での並置キャプチャ比較を成立させる（見た目軸の未計測を解消する唯一の実質的方法）
