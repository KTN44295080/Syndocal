...
| Static release chain | `pnpm.cmd --dir app run check:release:static` — PASS; completion ledger `23 Open + 8 Deferred + 27 Complete = 58`, Q1/Q4 `58/58` Flow references and `85` evidence records linked at this checkpoint, all listed static contracts passed. |
...
| Dedicated rendered browser gate | Fresh rerun with `CHROME_PATH=C:\Users\janua\AppData\Local\Google\Chrome\Application\chrome.exe` passed all four supported viewports (`3840x2160`, `2560x1440`, `1920x1080`, `1280x720`). Lighting, Video, Both, Timeline/nested-controls reachability, first-Escape focus return, and final CDP diagnostics passed at each viewport; every viewport reported `runtimeExceptions=0`, `runtimeConsoleErrors=0`, `logErrors=0`, `runtimeConsoleWarnings=0`, `logWarnings=0`, `harnessErrors=0`. |
...
| Current native maximized/F11 acceptance attempt | `pnpm.cmd --dir app run check:native-window` reached the isolated current-source QA executable and failed closed at the required maximized-client gate: last client size `1280x752`, below `1920x1000`. A live monitor query confirmed the only attached display is `1280x800` with `1280x752` work area; the required `1920x1080` monitor is unavailable in this environment. The QA process and port listener were cleaned up; no native interaction or physical output is claimed from this attempt. |
| Current-source native pane lifecycle retry | A reversible Win32 display-settings wrapper changed the primary mode from `2560x1600@180Hz/32bpp` to `1920x1080@180Hz/32bpp`, ran the pinned MSVC `14.44.35207` `check:native-window`, and restored the original mode. PASS: monitor/work area `1920x1080`/`1920x1008`, main max `1920x1008`, F11 `1920x1080`, Esc restore `1920x1008`, both detach orders, restart restore, reload adoption, direct child close, final reintegration, and 19 screenshots; wrapper exit `0`. |
| Current-source native Control surface probe | On the same physical `1920x1080@180Hz/32bpp` mode, the current checker passed the four Control tabs (`Lighting`, `Video`, `Both`, `Timeline`), ARIA tab state, one-visible-panel/zero-scroll contract, safe Video Audio In disclosure, empty-show recording guard, Both semantic blackout state, Timeline arranger, and lower-right Source shelf. Four Control screenshots and the JSON report were generated; no output, recording, Take, blackout, Arm, Take Over, or device action was dispatched. Display restored to `2560x1600@180Hz/32bpp`; isolated ports and QA processes were released. |

## Remaining boundary and next action

`UI-H5-CONTROL-001` remains **Open**. The browser and native process evidence establish the current-source Both surface and the responsive native executable; the latest native run also proves safe Control surface reachability and semantic structure on a physical `1920x1080` display. Native button-by-button interaction and dangerous workflow dispatch were not performed, and no NVDA/High Contrast/DPI/IME/reduced-motion matrix, physical DMX/Art-Net/sACN/USB/RDM/MIDI/OSC/video/display output, external client, two-machine, venue, soak, signing, or publication acceptance is claimed.
