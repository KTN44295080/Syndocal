...
| Effects | Ordered LFO/Position Wave/Color stack with reorder/bypass/clone/preset, BPM sync and graph-driven audio; independent Color runtime provides 2-8 stops, Cycle/Bounce/Sequence/seeded Random, RGB/HSV interpolation, fixture spread and RGB/RGBW/CMY/HSV/colored-wheel output; selective Cue ON/OFF Recall supports Effects Only and scoped Look updates; 13-preset library is filterable as Colour, Chaser, Move, Value, Curve, Mappings and Colour Mappings | Seven live engines (Colour, Chaser, Move, Value, Curve, Mappings, Colour Mappings), multiple types per engine and unlimited stack | The colour alias and Effect-state Cue recall gaps are closed, but Daslight still leads overall: six Syndocal library families still share LFO/Position Wave, per-Cue parameter morph/fade, calibrated multi-emitter colour and image/video mapping are absent, and unlimited practical stacking lacks release/venue proof |
...
2. Continue beyond the completed independent Color engine and selective Cue Effect recall: add independent Chaser, Move, Curve/Value and image/video Mapping runtimes with a fast visual rack, virtualized practical stacking, reorder, bypass, clone, preset and live phase/size/speed controls.
