...
- **T27-C アイコン化 ✅完了 2026-07-30**: トランスポート行（戻る/Pause Fade/Play Timeline等）と表示切替（キューエディター/マトリクス/キューパッド/ステータス）をアイコン+ホバーラベルへ（フェーダー列と同じ「アイコン+title」原則）。BO系は誤操作防止の観点でテキスト維持を検討（実装時判断・報告必須）。
