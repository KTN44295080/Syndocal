...
Updated: 2026-07-23 (T19-E re-audit; older addendum retained below)
Scope: Syndocal as a unified lighting and VJ application, preserving `.sdc v1` and the one-screen desk.

## Verdict

The software already has a broad, locally gated show-control core, but the declared Daslight-comparable lighting goal is not complete. T19-E calibrated multi-emitter colour is complete in software; expanded Scene Live operation, fixture onboarding, detachable workspaces/operator lock and final native/Art-Net evidence remain. Windows cross-application NDI and Spout, a physical DirectShow camera, and physical MIDI port opening have host evidence; unavailable hardware acceptance remains explicitly external.
...
| Lighting Cue Engine v2 | Complete locally; Daslight FX parity open | Cue Lists/Executors, tracking/block, Parts, IFCB Fade/Delay, Follow, MIB, HTP/LTP, Programmer/Blind, reference Palettes, ordered FX stack and seven-family/13-preset library; independent Color, Chaser, Move, Value, Curve, fixture-order Mapping and embedded image/text/video 2D Colour Mapping runtimes/editors; per-Cue-owned Effect optional same-ID state fade; GDTF-linked CIE xyY calibrated 3–16-emitter mixing with metadata-free extra emitters fail-closed; source-linked Scene Blocks; selective Effect Recall; DMX Input/Merge, serial output and RDM protocol tests | Physical/spectral Color/Chaser/Move/Curve/Mapping/Colour Mapping acceptance; Scene Block operator rehearsal with external timecode and a physical show rig; Art-Net/sACN/serial waveform, QLC+-class serial VID/PID and dual-port expansion, RDM fixtures/gateway, controller round trip |
...
| Performance/reliability | Calibrated path complete; controlled-host ordinary rerun and venue soaks open | Three one-hour release soaks pass. Calibrated RGB/Amber/Lime 200-fixture × 64-effect stack passes at p95 7.672 / p99 8.052 / max 9.660 ms and all-transition stress at p95 17.489 / p99 18.119 / max 18.390 ms. The unchanged ordinary 5/8/12 gate was marginally red on both pre/post same-host A/B (p95 5.113 vs 5.110 ms), proving non-increase but not an absolute pass; the earlier T19-D controlled state was green at 2.654 / 2.955 / 3.460 ms | Controlled-host ordinary gate rerun, representative multi-layer media/mixed ISF-HAP venue soak and sustained physical Art-Net-node capture |
