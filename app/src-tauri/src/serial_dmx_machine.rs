//! Machine-local USB-DMX selection.  Projects retain no COM/PnP identity.
...
/// admission. It lets the route layer undo a B selection if a worker fault or
/// zero-write-incomplete state is observed immediately after the atomic write.
/// This is never project data and never crosses Tauri IPC.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum SerialDmxMachineBindingFileSnapshot {
    Missing,
    Bytes(Vec<u8>),
}
...
pub fn capture_binding_file_snapshot(
    path: &Path,
) -> Result<SerialDmxMachineBindingFileSnapshot, String> {
    match fs::read(path) {
        Ok(bytes) => {
            if bytes.len() as u64 > MAX_SERIAL_DMX_MACHINE_BINDING_BYTES {
                return Err(
                    "Machine-local USB-DMX selection exceeds the safety limit before replacement"
                        .to_string(),
                );
            }
            Ok(SerialDmxMachineBindingFileSnapshot::Bytes(bytes))
        }
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            Ok(SerialDmxMachineBindingFileSnapshot::Missing)
        }
        Err(error) => Err(format!(
            "Unable to capture machine-local USB-DMX selection before replacement {}: {error}",
            path.display()
        )),
    }
}

pub fn restore_binding_file_snapshot(
    path: &Path,
    snapshot: &SerialDmxMachineBindingFileSnapshot,
) -> Result<(), String> {
    match snapshot {
        SerialDmxMachineBindingFileSnapshot::Missing => match fs::remove_file(path) {
            Ok(()) => Ok(()),
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(()),
            Err(error) => Err(format!(
                "Unable to remove rejected machine-local USB-DMX replacement {}: {error}",
                path.display()
            )),
        },
        SerialDmxMachineBindingFileSnapshot::Bytes(bytes) => {
            persist_binding_bytes_to_path(path, bytes)
        }
    }
}
...
fn persist_binding_bytes_to_path(path: &Path, bytes: &[u8]) -> Result<(), String> {
    if bytes.len() as u64 > MAX_SERIAL_DMX_MACHINE_BINDING_BYTES {
        return Err("Machine-local USB-DMX selection exceeds its safety limit".to_string());
    }
